...
namespace Himall.Service
{
    ...
    public partial class GiftService : ServiceBase, IGiftService
    {
        ...
        {
            var gift = Context.GiftInfo.FirstOrDefault(p => p.Id == id);
            if (gift == null)
        ...
        {
            QueryPageModel<GiftModel> result = new QueryPageModel<GiftModel>();
            result.Total = 0;
            ...
            int total = 0;
            datasql = datasql.GetPage(orderby, out total, query.PageNo, query.PageSize);

            //数据转换
            result.Models = datasql.ToList();
            result.Total = total;
        ...
        public IntegralMallAdInfo GetAdInfo(IntegralMallAdInfo.AdActivityType adtype, IntegralMallAdInfo.AdShowPlatform adplatform)
        {
            ...
            if (result != null)
            {
                ...
                var isExist = Context.WeiActivityInfo.Exist(d => d.Id == result.ActivityId && d.EndTime > nowdate && d.BeginTime <= nowdate);
                if (!isExist)
                {
                    return null;
                }
