...
namespace Himall.Service
{
    public class ConsultationService : ServiceBase, IConsultationService
    {
        ...
        {
            Context.ProductConsultationInfo.Remove(n => n.Id == id);
            Context.SaveChanges();
        }
        ...
        public QueryPageModel<ProductConsultationInfo> GetConsultations(ConsultationQuery query)
        {
            ...
            consultation = consultation.GetPage(d => d.OrderByDescending(o => o.Id), out total, query.PageNo, query.PageSize);
            QueryPageModel<ProductConsultationInfo> pageModel = new QueryPageModel<ProductConsultationInfo>() { Models = consultation.ToList(), Total = total };
            return pageModel;
        }

        public ProductConsultationInfo GetConsultation(long id)
        {
            return Context.ProductConsultationInfo.FirstOrDefault(p => p.Id == id);
        }
