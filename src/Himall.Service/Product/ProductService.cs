...
namespace Himall.Service
{
    ...
    {
        #region 平台服务
        public QueryPageModel<ProductInfo> GetProducts(ProductQuery productQueryModel)
        {
        ...
        {
            var sql = new Sql("SELECT count(0) FROM HiMall_Products WHERE Id=@0 AND IsDeleted=0", productId);

            var result = DbFactory.Default.ExecuteScalar<int>(sql);
        ...
        IQueryable<ProductInfo> GetProductsByQueryModel(ProductQuery productQueryModel, out int total)
        {
            ...
            return products.GetPage(d => d.OrderByDescending(o => o.Id), out total, productQueryModel.PageNo, productQueryModel.PageSize);
        }

        public void AuditProduct(long productId, ProductInfo.ProductAuditStatus auditStatus, string message)
        {
            ProductInfo product = Context.ProductInfo.FirstOrDefault(p => p.Id == productId);
            if (product == null)
        ...
        {
            var product = Context.ProductInfo.FirstOrDefault(p => p.Id == pId);
            if (null != product)
            {
        ...
        public void AddProduct(long shopId, ProductInfo product, string[] pics, SKUInfo[] skus, ProductDescriptionInfo description, ProductAttributeInfo[] attributes, long[] goodsCategory, SellerSpecificationValueInfo[] sellerSpecifications, ProductLadderPricesInfo[] prices)
        {
            //设置商品基本属性
            ...
            if (product.IsOpenLadder)
            {
                prices = prices.Select(p =>
                {
                    p.ProductId = product.Id;
                    return p;
                }).ToArray();
                Context.ProductLadderPricesInfo.AddRange(prices);
            }
        ...
        {
            var product = Context.ProductInfo.FirstOrDefault(p => p.Id == model.Id);
        ...
        public bool ApplyForSale(long id)
        {
            ...
            var product = Context.ProductInfo.FirstOrDefault(p => p.Id == id);
        ...
        public void UpdateProduct(ProductInfo product, string[] pics, SKUInfo[] skus, ProductDescriptionInfo description, ProductAttributeInfo[] attributes, long[] goodsCategory, SellerSpecificationValueInfo[] sellerSpecifications, ProductLadderPricesInfo[] prices)
        {
        ...
        public ProductInfo GetProductCache(long id)
        {
            ...
            var sql = new Sql("select * from himall_Products where Id=@0  AND IsDeleted=0 ", id);
            result = DbFactory.Default.FirstOrDefault<ProductInfo>(sql);//, new { Id = id });
        ...
        public ProductInfo GetNeedRefreshProductInfo(long id)
        {
            ...
            var sql = new Sql("select MinSalePrice, SaleCounts, MeasureUnit,SaleStatus,AuditStatus,FreightTemplateId,ShopId,TypeId,IsHadHPV from himall_products where Id=@0", id);

            p = DbFactory.Default.FirstOrDefault<ProductInfo>(sql);//, new { Id = id });
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public ProductInfo GetInsurance(long id)
        {
            ProductInfo p = new ProductInfo();
            var sql = new Sql("select HPVY, HPVO, IsHadHPV from himall_products where Id=@0", id);
        ...
        public AttributeInfo GetAttributeInfo(long attrId)
        {
            ...
            var sql = new Sql("SELECT * FROM HiMall_Attributes");
            listAttr = DbFactory.Default.Query<AttributeInfo>(sql).ToList();
            sql = new Sql("SELECT * FROM himall_attributevalues");
            listAttrVal = DbFactory.Default.Query<AttributeValueInfo>(sql).ToList();
        ...
        public List<SKUInfo> GetSkuList(long productId)
        {
            ...
            list = DbFactory.Default.Query<SKUInfo>(sql).ToList();//, new { ProductId = productId }).ToList();
            return list;
        }
        ...
        public void SaveSellerSpecifications(List<SellerSpecificationValueInfo> info)
        {
            if (null != info && info.Count() > 0)
            {
                ...
                foreach (var item in info)
                {
                    ...
                    {
                        item.Specification = Context.SpecificationValueInfo.FirstOrDefault(p => p.Id == item.ValueId).Specification;
        ...
        {
            var products = NoLazyNoProxyContext.ProductInfo.Where(item => ids.Contains(item.Id) && item.SaleStatus == ProductInfo.ProductSaleStatus.OnSale && item.AuditStatus == ProductInfo.ProductAuditStatus.Audited && item.IsDeleted == false);
            return products;
        }
        public List<ProductInfo> GetAllProductByIds(IEnumerable<long> ids)
        {
            var sql = new Sql("select * from himall_products where Id in (@0)  AND IsDeleted=0", ids);

            var list = DbFactory.Default.Query<ProductInfo>(sql).ToList();//, new { ProductId = ids.ToArray() }).ToList();
            return list;
        }
        ...
        public QueryPageModel<ProductInfo> SearchProduct(ProductSearch search)
        {
            ...
            {
                result = result.Where(d => d.IsOpenLadder == false);
            }
            ...
            if (maxpage < search.PageNumber)
            {
                ...
                QueryPageModel<ProductInfo> noDataResult = new QueryPageModel<ProductInfo>()
                {
                    Total = total,
                    Models = (new ProductInfo[] { }).ToList()
                };
                return noDataResult;
            }
            ...
            {
                Total = total,
                Models = productsResult.ToList()
            };
            return pageModel;
        }


        public QueryPageModel<ProductInfo, SearchProductModel> SearchProductAndOtherModel(ProductSearch search)
        {
            ...
            if (maxpage < search.PageNumber)
            {
                ...
                QueryPageModel<ProductInfo, SearchProductModel> noDataResult = new QueryPageModel<ProductInfo, SearchProductModel>()
                {
                    Total = total,
                    Models = (new ProductInfo[] { }).ToList(),
                    TotalData = null
            ...
            QueryPageModel<ProductInfo, SearchProductModel> pageModel = new QueryPageModel<ProductInfo, SearchProductModel>()
            {
                Total = total,
                Models = productsResult.ToList(),
        ...
        public IQueryable<ProductInfo> GetNewSaleProduct(long shopId, int count = 5)
        {
            ...
            if (Cache.Exists(CACHE_MANAGER_KEY))
            {
                return (Core.Cache.Get<List<ProductInfo>>(CACHE_MANAGER_KEY)).AsQueryable();
            }
            else
            {
                var data =
        ...
        public IQueryable<ProductInfo> GetHotConcernedProduct(long shopId, int count = 5)
        {
            ...
            if (Cache.Exists((CACHE_MANAGER_KEY)))
            {
                return (Core.Cache.Get<List<ProductInfo>>(CACHE_MANAGER_KEY)).AsQueryable().OrderByDescending(p => p.ConcernedCount);
            }
            else
            {
                var data = (
        ...
        public QueryPageModel<FavoriteInfo> GetUserConcernProducts(long userId, int pageNo, int pageSize)
        {
            ...
            QueryPageModel<FavoriteInfo> pageModel = new QueryPageModel<FavoriteInfo>()
            {
                Models = favorite.ToList(),
                Total = total
            };
            return pageModel;
        }
        ...
        public bool UpdateStockByProId(long pid, int stock)
        {
            var skus = Context.SKUInfo.Where(e => e.ProductId == pid);
            if (skus.Count() == 0)
        ...
        public bool IsFreeRegion(long productId, decimal discount, int streetId, int count, string skuId)
        {
            ...
            if (productInfo != null)
            {
                ...
                {
                    var skus = GetSkuList(productId).Where(s => s.Stock >= 0).ToList();//获取有效SKU
                ...
                var regions = freeRegions.Where(p => p.RegionSubList.Contains(streetId)).ToList();//根据模板设置的包邮地区过滤出当前配送地址所在地址
        ...
        {
            List<SKUInfo> list = new List<SKUInfo>();
            //var sql = new Sql(" select * from himall_skus ");
            //if (skuIds.Any())
            //{
            //    sql.Append(" WHERE ");
            //    var flag = false;
            //    foreach (var str in skuIds)
            //    {
            //        if (flag) sql.Append(" OR ");
            //        sql.Append(" Id = @0 ", str);
            //        flag = true;
            //    }
            //    list = DbFactory.Default.Query<SKUInfo>(sql).ToList();//, new { SkuId = skuIds.ToArray() }).ToList();
            //}
            if (skuIds.Any())
            {
                var sql = new Sql("select * from himall_skus where Id in (@0)", skuIds);
        ...
        {
            return this.NoLazyNoProxyContext.ProductInfo.Where(p => p.SaleStatus == ProductInfo.ProductSaleStatus.OnSale && p.AuditStatus == ProductInfo.ProductAuditStatus.Audited && p.CategoryId == categoryId && p.IsDeleted == false)
