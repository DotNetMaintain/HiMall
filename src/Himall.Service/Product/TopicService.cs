...
namespace Himall.Service
{
    public class TopicService : ServiceBase, ITopicService
    {
        ...
        public QueryPageModel<TopicInfo> GetTopics(int pageNo, int pageSize, PlatformType platformType = PlatformType.PC)
        {
            ...
            int total;
            topic.Models = Context.TopicInfo.FindBy(item => item.PlatForm == platformType, pageNo, pageSize, out total, item => item.Id, false).ToList();
            topic.Total = total;
            return topic;
        }
        ...
        public QueryPageModel<TopicInfo> GetTopics(TopicQuery topicQuery)
        {
            ...
            topic.Models = topics.GetPage(orderBy, out total, topicQuery.PageNo, topicQuery.PageSize).ToList();
            topic.Total = total;
            return topic;
        }
        ...
        {
            //删除数据
            Context.TopicInfo.Remove(n => n.Id == id);
        ...
        {
            var topic = Context.TopicInfo.FirstOrDefault(p => p.Id == id);
            if (topic != null)
            {
        ...
        public void UpdateTopic(TopicInfo topicInfo)
        {
            ...
            try
            {
                ...
                //保存数据
                TopicInfo newTopicInfo = Context.TopicInfo.FirstOrDefault(p => p.Id == topicInfo.Id);
                newTopicInfo.Name = topicInfo.Name;
                newTopicInfo.BackgroundImage = !string.IsNullOrEmpty(topicInfo.BackgroundImage) ? topicInfo.BackgroundImage : newTopicInfo.BackgroundImage;
                newTopicInfo.TopImage = !string.IsNullOrEmpty(topicInfo.TopImage) ? topicInfo.TopImage : newTopicInfo.TopImage;
                newTopicInfo.FrontCoverImage = !string.IsNullOrEmpty(topicInfo.FrontCoverImage) ? topicInfo.FrontCoverImage : newTopicInfo.FrontCoverImage;
