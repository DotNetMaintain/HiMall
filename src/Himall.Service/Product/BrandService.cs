    ...
    public class BrandService : ServiceBase, IBrandService
    {
        ...
        public void DeleteBrand(long id)
        {
            ...
            if (brand != null)
            {
                ...
                var apply = Context.ShopBrandApplysInfo.Where(item => item.BrandName.ToLower().Trim() == brand.Name.ToLower().Trim());
                if (apply.Any())
                {
                    Context.ShopBrandApplysInfo.RemoveRange(apply);
                    foreach (var shopbrand in apply.ToList())
                    {
                        //删除对应图片
                        if (shopbrand.ApplyMode == (int)ShopBrandApplysInfo.BrandApplyMode.New)
                        {
        ...
        public QueryPageModel<BrandInfo> GetBrands(string keyWords, int pageNo, int pageSize)
        {
            ...
            brands = brands.GetPage(p => p.OrderByDescending(pp => pp.Id), out total, pageNo, pageSize);
        ...
        public void AuditBrand(long id, Himall.Model.ShopBrandApplysInfo.BrandAuditStatus status, string remark)
        {
            var m = Context.ShopBrandApplysInfo.FirstOrDefault(p => p.Id == id);
        ...
        {            
            ...
            {
                var brand = Context.ShopBrandApplysInfo.Where(p => p.ShopId == shopId);
                if (brand.Any())
                {
                    var brandids = brand.Select(b => b.BrandId.Value);
                    result = result.Where(p => !brandids.Contains(p.Id));
                }
            }
            #endregion
        ...
        public void UpdateApplyBrand(ShopBrandApplysInfo model)
        {
            var data = Context.ShopBrandApplysInfo.FirstOrDefault(d => d.Id == model.Id);
            if (data == null)
            {
        ...
        public ShopBrandApplysInfo GetExistApply(long shopId, string brandName)
        {
            var apply = Context.ShopBrandApplysInfo.Where(item => item.ShopId == shopId && item.BrandName.ToLower().Trim() == brandName.ToLower().Trim()).FirstOrDefault();
            if (apply != null)
                return apply;
            else
                return null;
        ...
        public QueryPageModel<ShopBrandApplysInfo> GetShopBrandApplys(long? shopId, int? auditStatus, int pageNo, int pageSize, string keyWords)
        {
            int total = 0;
            ...
            if (shopId.HasValue)
            {
                brands = brands.Where(item => item.ShopId == shopId && (item.Himall_Brands.IsDeleted == false || item.BrandId == null));
            }
            if (!string.IsNullOrEmpty(keyWords))
            {
                brands = brands.Where(item => item.Himall_Shops.ShopName.Contains(keyWords));
            }
            brands = brands.GetPage(d => d.OrderByDescending(o => o.Id), out total, pageNo, pageSize);
            //if (auditStatus != null)
            //{
            ...
            QueryPageModel<ShopBrandApplysInfo> pageModel = new QueryPageModel<ShopBrandApplysInfo>() { Models = brands.ToList(), Total = total };
            return pageModel;
        }
        ...
        {
            var model = Context.ShopBrandApplysInfo.FirstOrDefault(p => p.Id == id);
