...
namespace Himall.Service
{
    public class CommentService : ServiceBase, ICommentService
    {
        ...
        public QueryPageModel<Model.ProductCommentInfo> GetComments(CommentQuery query)
        {
            int total = 0;
            ...
            consultation = consultation.GetPage(orderby, out total, query.PageNo, query.PageSize);
            QueryPageModel<ProductCommentInfo> pageModel = new QueryPageModel<ProductCommentInfo>() { Models = consultation.ToList(), Total = total };
            return pageModel;
        }

        public Model.ProductCommentInfo GetComment(long id)
        {
            return Context.ProductCommentInfo.FirstOrDefault(p => p.Id == id);
        }
        ...
        public QueryPageModel<UserOrderCommentModel> GetOrderComment(OrderCommentQuery query)
        {
            ...
            int total = 0;
            model = model.GetPage(d => d.OrderByDescending(item => item.Id), out total, query.PageNo, query.PageSize);
        ...
        /// <returns></returns>
        public QueryPageModel<ProductEvaluation> GetProductEvaluation(CommentQuery query)
        {
            int total = 0;
            ...
            order = order.GetPage(orderBy, out total, query.PageNo, query.PageSize);
            ...
            QueryPageModel<ProductEvaluation> pageModel = new QueryPageModel<ProductEvaluation>()
            {
                Models = model.ToList(),
                Total = total
            };
        ...
        public QueryPageModel<ProductEvaluation> GetProductEvaluationTwo(CommentQuery query)
        {
            int total = 0;
            IQueryable<OrderItemInfo> order = Context.OrderItemInfo.FindBy(a => a.OrderInfo.UserId == query.UserID && a.OrderInfo.OrderStatus == Himall.Model.OrderInfo.OrderOperateStatus.Finish);
            var orderBy = order.GetOrderBy(d => d.OrderByDescending(o => o.Id));
        ...
        public List<ProductEvaluation> GetProductEvaluationByOrderId(long orderId, long userId)
        {
            var order = Context.OrderItemInfo.Where(a => a.OrderId == orderId && a.OrderInfo.UserId == userId && a.OrderInfo.OrderStatus == Himall.Model.OrderInfo.OrderOperateStatus.Finish)
                .OrderBy(a => Context.ProductCommentInfo.Any(b => b.ProductId == a.ProductId && b.UserId == userId && b.SubOrderId == a.Id)).ToList();
            var orderDate = DateTime.Now;
            if (null == order || order.Count() <= 0)
            {
                var flag = ServiceProvider.Instance<ISiteSettingService>.Create.GetSiteSettings().IsOpenHistoryOrder;
                if (flag)
                {
                    ...
                    if (null != o)
                    {
                        ...
                        order = DbFactory.MongoDB.AsQueryable<OrderItemInfo>().Where(a => a.OrderId == orderId).AsEnumerable().OrderBy(a => Context.ProductCommentInfo.Any(b => b.ProductId == a.ProductId && b.UserId == userId && b.SubOrderId == a.Id)).ToList();
                    }
                }
            }
            var model = order.Select(a => new ProductEvaluation
            {
                ...
                ThumbnailsUrl = a.ThumbnailsUrl,
                ProductName = a.ProductName,
                BuyTime = null == a.OrderInfo ? orderDate : a.OrderInfo.OrderDate,
        ...
        {
            var productcomments = Context.ProductCommentInfo.FindBy(c => c.ProductId.Equals(productId)).Where(a => (!a.IsHidden.HasValue || a.IsHidden.Value == false));
        ...
        public decimal GetProductMark(long id)
        {
            ...
            var result = 5M;
            var sql = new Sql("SELECT AVG(ReviewMark) from himall_productcomments where ProductId = @0", id);

            result = DbFactory.Default.ExecuteScalar<decimal>(sql);
        ...
        {
            QueryPageModel<ProductCommentInfo> result = new QueryPageModel<ProductCommentInfo>();
            var sql = BuildProductCommentQueryable(query.ProductId, query.ShopId, query.ShopBranchId, query.ShowHidden);
            if (query.CommentType.HasValue)
            {
                switch (query.CommentType)
                {
                    case ProductCommentMarkType.Low:
                        sql = sql.Where(d => d.ReviewMark <= (int)ProductCommentMarkType.Low);
                        break;
                    case ProductCommentMarkType.Medium:
                        sql = sql.Where(d => d.ReviewMark == (int)ProductCommentMarkType.Medium);
                        break;
                    case ProductCommentMarkType.High:
                        sql = sql.Where(d => d.ReviewMark >= (int)ProductCommentMarkType.High);
                        break;
                    case ProductCommentMarkType.HasImage:
                        sql = sql.Where(d => d.Himall_ProductCommentsImages.Any());
                        break;
                    case ProductCommentMarkType.Append:
                        sql = sql.Where(d => d.AppendDate.HasValue);
                        break;
                }
            }
            var orderby = sql.GetOrderBy(o => o.OrderByDescending(d => d.ReviewDate));
            int total = 0;
            result.Models = sql.GetPage(orderby, out total, query.PageNo, query.PageSize).ToList();
            result.Total = total;
            return result;
        }
        /// <summary>
        ...
        /// <returns></returns>
        public ProductCommentCountAggregateModel GetProductCommentCountAggregate(long? productId = null, long? shopId = null, long? shopBranchId = null)
        {
            ProductCommentCountAggregateModel result = new ProductCommentCountAggregateModel();
            var sql = BuildProductCommentQueryable(productId, shopId, shopBranchId, false);
            result.AllComment = sql.Count();
            result.LowComment = sql.Count(d => d.ReviewMark <= (int)ProductCommentMarkType.Low);
            result.MediumComment = sql.Count(d => d.ReviewMark == (int)ProductCommentMarkType.Medium);
            result.HighComment = sql.Count(d => d.ReviewMark >= (int)ProductCommentMarkType.High);
            result.HasImageComment = sql.Count(d => d.Himall_ProductCommentsImages.Any());
            result.AppendComment = sql.Count(d => d.AppendDate.HasValue);
            return result;
        }
        ...
        public int GetProductHighCommentCount(long? productId = null, long? shopId = null, long? shopBranchId = null)
        {
            ProductCommentCountAggregateModel result = new ProductCommentCountAggregateModel();
            var sql = BuildProductCommentQueryable(productId, shopId, shopBranchId, false);
            return sql.Count(d => d.ReviewMark >= (int)ProductCommentMarkType.High);
        }

        private IQueryable<ProductCommentInfo> BuildProductCommentQueryable(long? productId = null, long? shopId = null, long? shopBranchId = null, bool ShowHidden = false)
        {
            var sql = Context.ProductCommentInfo.AsQueryable();
            if (productId.HasValue)
            {
                sql = sql.Where(d => d.ProductId == productId.Value);
            }
            if (!ShowHidden)
            {
                sql = sql.Where(d => !d.IsHidden.HasValue || d.IsHidden.Value == false);
            }
            if (shopId.HasValue)
            {
                sql = sql.Where(d => d.ShopId == shopId.Value);
            }
            if (shopBranchId.HasValue)
            {
                var ordsql = Context.OrderInfo.Where(d => d.ShopBranchId == shopBranchId.Value).Select(d => d.Id);
                var orditemsql = Context.OrderItemInfo.Where(d => ordsql.Contains(d.OrderId)).Select(d => (long?)d.Id);
                sql = sql.Where(d => orditemsql.Contains(d.SubOrderId));
            }
