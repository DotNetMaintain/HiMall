...
namespace Himall.Service
{
	public class ShopCategoryService : ServiceBase, IShopCategoryService
	{
        ...
        public List<ShopCategoryInfo> GetCategorysByProductId(long id)
        {
            ...
            var model = Context.ProductShopCategoryInfo.Where(p => p.ProductId == id);
            if (model.Any())
            {
                return model.Select(s => s.ShopCategoryInfo).ToList();
            }
            else
            {
                return new List<ShopCategoryInfo>()
                {
                    new ShopCategoryInfo() { Name = "" }
                };
            }
        }
		...
		public void UpdateCategoryName(long id, string name)
		{
			...
			var category = Context.ShopCategoryInfo.FirstOrDefault(p => p.Id == id);
		...
		{
			var category = Context.ShopCategoryInfo.FirstOrDefault(p => p.Id == model.Id);
		...
		private void ProcessingDeleteCategory(long id, long shopId)
		{
			...
			if (subIds.Count() == 0)
			{
                Context.ProductShopCategoryInfo.RemoveRange(Context.ProductShopCategoryInfo.Where(n => n.ShopCategoryId == id));
				Context.ShopCategoryInfo.Remove(Context.ShopCategoryInfo.FirstOrDefault(p => p.Id == id));
				return;
			}
			else
			{
		...
		{
			var isSelf = Context.ShopInfo.FirstOrDefault(p => p.Id == shopId).IsSelf;
