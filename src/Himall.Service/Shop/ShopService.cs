...
namespace Himall.Service
{
    public class ShopService : ServiceBase, IShopService
    {
        ...
        #region 实现店铺服务
        public QueryPageModel<ShopInfo> GetAuditingShops(ShopQuery shopQueryModel)
        {
            return null;
        }

        public QueryPageModel<ShopInfo> GetSellers(SellerQuery sellerQueryModel)
        {
        ...
        public QueryPageModel<ShopInfo> GetShops(ShopQuery shopQueryModel)
        {
            ...
            int total;
            shops = shops.GetPage(orderBy, out total, shopQueryModel.PageNo, shopQueryModel.PageSize);
        ...
        {
            var shops = Context.ShopInfo.Where(x => x.ShopStatus != ShopInfo.ShopAuditStatus.WaitAudit).ToList();
            shops = shops.Where(x => x.ShopStatus != ShopInfo.ShopAuditStatus.Refuse).ToList();
            shops = shops.Where(x => x.ShopStatus != ShopInfo.ShopAuditStatus.Unusable).ToList();
            shops = shops.Where(x => x.ShopStatus != ShopInfo.ShopAuditStatus.WaitConfirm).ToList();
            return shops;
        }
        ...
        public ShopInfo GetShop(long id, bool businessCategoryOn = false)
        {
            ...
            var shop = NoLazyNoProxyContext.ShopInfo.FirstOrDefault(p => p.Id == id);//因外键导致循环引用，所以去掉延迟加载，避免REDIS缓存序列化报错
            var vShop = Context.VShopInfo.Where(e => e.ShopId == id);
            if (null == shop)
                return null;
            ...
            shop.Himall_VShop = vShop.ToList().Select(e => new VShopInfo
            {
                BackgroundImage = e.BackgroundImage,
                buyNum = e.buyNum,
                CreateTime = e.CreateTime,
                Description = e.Description,
                HomePageTitle = e.HomePageTitle,
                Id = e.Id,
                Logo = e.Logo,
                Name = e.Name,
                ShopId = e.ShopId,
                ShowSequence = e.ShowSequence,
                State = e.State,
                Tags = e.Tags,
                VisitNum = e.VisitNum,
                WXLogo = e.WXLogo
        ...
        {
            var shop = Context.ShopInfo.FirstOrDefault(p => p.Id == id);
            return shop;
        }

        public void DeleteShop(long id)
        {
            Context.ShopInfo.Remove(Context.ShopInfo.FirstOrDefault(p => p.Id == id));
            Context.SaveChanges();
        }
        ...
        public void UpdateShop(ShopInfo shop)
        {
            ...
            actual.ProvideInvoice = shop.ProvideInvoice ?? actual.ProvideInvoice;//商家是否提供发票
        ...
        {
            var c = Context.CategoryInfo.FirstOrDefault(p => p.Id == id);
        ...
        public PlatConsoleModel GetPlatConsoleMode()
        {
            ...
            model.ProductNum = products.Where(a => a.SaleStatus != ProductInfo.ProductSaleStatus.InDraft).Count();
            model.OnSaleProducts = products.Where(a => a.SaleStatus != ProductInfo.ProductSaleStatus.InDraft && a.AuditStatus == ProductInfo.ProductAuditStatus.Audited).Count();
        ...
        public SellerConsoleModel GetSellerConsoleModel(long shopId)
        {
            ...
            var shopG = Context.ShopGradeInfo.FirstOrDefault(p => p.Id == shopinfo.GradeId);
            ...
            var comment = Context.ProductCommentInfo.Where(a => a.ShopId == shopId && (!a.ReplyDate.HasValue));
            var consultations = Context.ProductConsultationInfo.Where(a => a.ShopId == shopId && (!a.ReplyDate.HasValue));
        ...
        public bool IsExpiredShop(long shopId)
        {
            ...
            var shopinfo = Context.ShopInfo.FirstOrDefault(p => p.Id == shopId);
            //店铺过期时间以晚上24点之前为节点，与商品自动下架同步
            if ((shopinfo.EndDate.HasValue && shopinfo.EndDate.Value.Date.AddDays(1) <= date) || shopinfo.ShopStatus == ShopInfo.ShopAuditStatus.HasExpired)
                return true;
            else
                return false;
        }
        ...
        {
            return Context.ShopGradeInfo.FirstOrDefault(p => p.Id == id);
        }
        ...
        public void DeleteShopGrade(long id, out string msg)
        {
            ...
            Context.ShopGradeInfo.Remove(Context.ShopGradeInfo.FirstOrDefault(p => p.Id == id));
            Context.SaveChanges();
        }

        public void UpdateShopGrade(ShopGradeInfo shopGrade)
        {
            var actual = Context.ShopGradeInfo.FirstOrDefault(p => p.Id == shopGrade.Id);
        ...
        public long GetShopSpaceUsage(long shopId)
        {
            ...
            var limit = Context.ShopGradeInfo.FirstOrDefault(p => p.Id == shop.GradeId).ImageLimit;
        ...
        public long GetShopConcernedCount(long shopId)
        {
            ...
            {
                ShopConcernedCount = Core.Cache.Get<long>(CACHE_MANAGER_KEY);
            }
            else
            {
        ...
        {
            Context.ShopInfo.FirstOrDefault(p => p.Id == shopId).Logo = img;
            Context.SaveChanges();
        }
        ...
        public ShopInfo.ShopVistis GetShopVistiInfo(DateTime startDate, DateTime endDate, long shopId)
        {
            ShopInfo.ShopVistis shopVistis = (
                ...
                select new ShopInfo.ShopVistis
                {
                    ...
                    OrderPayCount = g.Sum(c => c.OrderPayCount),
                }
                    ).FirstOrDefault();
            if (shopVistis != null)
            {
        ...
        public void ShopReNew(long shopid, int year)
        {
            ...
            if (shopinfo != null)
            {
                ...
                shopinfo.ShopStatus = ShopInfo.ShopAuditStatus.Open;
                Context.SaveChanges();
            }
        }
        ...
        public QueryPageModel<ShopRenewRecord> GetShopRenewRecords(ShopQuery query)
        {
            ...
            QueryPageModel<ShopRenewRecord> pageModel = new QueryPageModel<ShopRenewRecord>() { Models = complaints.ToList(), Total = total };
            return pageModel;
        }
        ...
        public QueryPageModel<BusinessCategoriesApplyInfo> GetBusinessCateApplyList(BussinessCateApplyQuery query)
        {
            int total = 0;
            ...
            // var favorite = context.FavoriteShopInfo.FindBy(a => a.UserId == userId, pageNo, pageSize, out total, a => a.Id, false);
            QueryPageModel<BusinessCategoriesApplyInfo> pageModel = new QueryPageModel<BusinessCategoriesApplyInfo>()
            {
        ...
        public void SaleOffAllProduct(long id)
        {
            ...
            Context.Database.ExecuteSqlCommand("UPDATE Himall_ShopBranchSkus SET Status = 1 WHERE Status = 0 AND ShopId =" + id);
        }
        ...
        public ShopInfo GetSelfShop()
        {
            ...
            result = DbFactory.Default.FirstOrDefault<ShopInfo>(sql);
        ...
        {
            return Context.ShopWdgjSetting.Where(a => a.uCode == uCode).FirstOrDefault();
        }

        public ShopWdgjSetting GetshopWdgjInfoById(long shopId)
        {
            return Context.ShopWdgjSetting.Where(a => a.ShopId == shopId).FirstOrDefault();
        }
        /// <summary>
        /// 更新网店管家参数
        /// </summary>
        ...
        {
            if (Context.ShopWdgjSetting.Any(d => d.uCode == wdgj.uCode && d.Id != wdgj.Id))
            {
                throw new HimallException("接入码己存在");
            }
            var shop = Context.ShopWdgjSetting.FirstOrDefault(p => p.Id == wdgj.Id);
            shop.uCode = wdgj.uCode;
            shop.uSign = wdgj.uSign;
            Context.SaveChanges();
        }
        /// <summary>
        /// 增加网店管家参数
        ...
        {
            if (Context.ShopWdgjSetting.Any(d => d.uCode == wdgj.uCode))
            {
                throw new HimallException("接入码己存在");
            }
            Context.ShopWdgjSetting.Add(wdgj);
            Context.SaveChanges();
        }
        public void UpdateShopPrint(ShopInfo info)
        {
            if (info != null)
            {
                var shopInfo = Context.ShopInfo.FirstOrDefault(a => a.Id == info.Id);
                if (shopInfo != null)
                {
                    shopInfo.PrintCount = info.PrintCount;
                    shopInfo.IsAutoPrint = info.IsAutoPrint;
                }
                Context.SaveChanges();
            }
        }
