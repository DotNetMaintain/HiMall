...
namespace Himall.Service
{
    public class SlideAdsService : ServiceBase, ISlideAdsService
    {
        ...
        public IEnumerable<Model.SlideAdInfo> GetSlidAdsByTypes(long shopId, IEnumerable<SlideAdInfo.SlideAdType> types)
        {
            return Context.SlideAdInfo.Where(s => s.ShopId == shopId && types.Contains(s.TypeId)).OrderBy(t => t.DisplaySequence).ToList();
        }
        ...
        {
            IEnumerable<SlideAdInfo> sliAdInfos = Context.SlideAdInfo.FindBy(i => i.ShopId == shopId).OrderBy(i => i.Id);
            if (slidAdsType != SlideAdInfo.SlideAdType.Initial)
            {
                sliAdInfos = sliAdInfos.Where(item => item.TypeId == slidAdsType);
            }
            if (sliAdInfos.Count() == 0)
            {
                SlideAdInfo[] newSlideAdInfos = new SlideAdInfo[5];
                for (int i = 0; i < 5; i++)
                {
                    newSlideAdInfos[i] = new SlideAdInfo() { ImageUrl = "", ShopId = shopId, Url = "", TypeId = slidAdsType.Value };
                }
                Context.SlideAdInfo.AddRange(newSlideAdInfos);
                Context.SaveChanges();
                sliAdInfos = newSlideAdInfos;
            }
            return sliAdInfos;
        }
        ...
        public void AdjustSlidAdIndex(long shopId, long id, bool direction, SlideAdInfo.SlideAdType type)
        {
            ...
            if (direction)
            {
                ...
                var prev = Context.SlideAdInfo.Where(s => s.ShopId == shopId && s.TypeId == type).OrderByDescending(s => s.DisplaySequence).FirstOrDefault(s => s.DisplaySequence < slide.DisplaySequence);
                if (null != prev)
                {
            ...
            {
                var next = Context.SlideAdInfo.Where(s => s.ShopId == shopId && s.TypeId == type).OrderBy(s => s.DisplaySequence).FirstOrDefault(s => s.DisplaySequence > slide.DisplaySequence);
