...
namespace Himall.Service
{
    public class MobileHomeProductsService : ServiceBase, IMobileHomeProductsService
    {
        ...
        {
            Context.MobileHomeProductsInfo.Remove(n => n.Id == id);
            Context.SaveChanges();
        }

        public void DeleteList(long[] ids)
        {
            var model = Context.MobileHomeProductsInfo.FindBy(item => ids.Contains(item.Id));
            Context.MobileHomeProductsInfo.RemoveRange(model);
            Context.SaveChanges();
        }

        public QueryPageModel<Model.MobileHomeProductsInfo> GetMobileHomePageProducts(long shopId, PlatformType platformType, ProductQuery productQuery)
        {
            ...
            if (!string.IsNullOrWhiteSpace(productQuery.ShopName))
            {
                ...
                var shopIds = Context.ShopInfo.FindBy(item => item.ShopName.Contains(productQuery.ShopName)).Select(item => item.Id);
                homeProducts = homeProducts.Where(item => item.Himall_Products.Himall_Shops.ShopName.Contains(productQuery.ShopName) || shopIds.Contains(item.Himall_Products.ShopId));
            }
            ...
            QueryPageModel<MobileHomeProductsInfo> pageModel = new QueryPageModel<MobileHomeProductsInfo>() { Models = homeProducts.ToList(), Total = total };
            return pageModel;
        }

        public QueryPageModel<Model.MobileHomeProductsInfo> GetSellerMobileHomePageProducts(long shopId, PlatformType platformType, ProductQuery productQuery)
        {
            var homeProducts = Context.MobileHomeProductsInfo
                .FindBy(item => item.ShopId == shopId && item.PlatFormType == platformType);
