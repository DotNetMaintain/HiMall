...
namespace Himall.Service
{
    public class FloorService : ServiceBase, IFloorService
    {
        ...
        public void UpdateFloorBasicInfo(long homeFloorId, string name, IEnumerable<long> topLevelCategoryIds)
        {
            ...
            var homeFloorBasicInfo = Context.HomeFloorInfo.FirstOrDefault(p => p.Id == homeFloorId);
        ...
        {
            var homeFloor = Context.HomeFloorInfo.FirstOrDefault(p => p.Id == homeFloorId);
        ...
        {
            return Context.HomeFloorInfo.FirstOrDefault(p => p.Id == id);
        }
        ...
        public void DeleteHomeFloor(long id)
        {
            ...
            Context.HomeFloorInfo.Remove(n => n.Id == id);
            Context.SaveChanges();
            RemoveImage(id);//删除图片
        }
        ...
        public void UpdateHomeFloorDetail(HomeFloorInfo homeFloor)
        {
            ...
            if (homeFloor.Id == 0)
            {
                ...
                floorId = Context.HomeFloorInfo.Add(homeFloor).Id;
            }
            else
            {
                var f = Context.HomeFloorInfo.FirstOrDefault(p => p.Id == homeFloor.Id);
                f.DefaultTabName = homeFloor.DefaultTabName;
                f.FloorName = homeFloor.FloorName;
                f.SubName = homeFloor.SubName;
                f.StyleLevel = homeFloor.StyleLevel;
                f.CommodityStyle = homeFloor.CommodityStyle;
                f.DisplayMode = homeFloor.DisplayMode;
            }
        ...
        void UpdateProductModule(HomeFloorInfo homeFloor)
        {
            ...
            var oriHomeFloor = Context.HomeFloorInfo.FirstOrDefault(p => p.Id == homeFloor.Id);
            int i = 0;
