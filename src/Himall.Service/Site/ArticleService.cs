...
namespace Himall.Service
{
    public class ArticleService : ServiceBase, IArticleService
    {
        ...
        public QueryPageModel<ArticleInfo> Find(long? articleCategoryId, string titleKeyWords, int pageSize, int pageNumber, bool isShowAll = true)
        {
        ...
        public IQueryable<ArticleInfo> GetTopNArticle<T>(int num, long categoryId, Expression<Func<ArticleInfo, T>> sort = null, bool isAsc = false)
        {
            ...
            var query = Context.ArticleInfo.FindBy(a => (a.CategoryId == categoryId || cate.Contains(a.CategoryId)) && a.IsRelease == true);
            if (sort == null)
            {
                if (isAsc)
        ...
        public IQueryable<ArticleInfo> GetArticleByArticleCategoryIds(List<long> articleCategoryIds)
        {
            return Context.ArticleInfo.Where(item => articleCategoryIds.Contains(item.CategoryId));
        }
