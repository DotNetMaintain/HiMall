...
namespace Himall.Service
{
    public class WeixinMenuService : ServiceBase, IWeixinMenuService
    {
        ...
        public void UpdateMenu(MenuInfo model)
        {
            ...
            if (model.Title.Length == 0 || (model.Title.Length > 7 && model.ParentId != 0))
                throw new Himall.Core.HimallException("二级菜单的名称不能为空且在5个字符以内");
            var menu = Context.MenuInfo.FirstOrDefault(p => p.Id == model.Id);
        ...
        {
            return Context.MobileFootMenuInfo;
        }

        public Model.MobileFootMenuInfo GetFootMenusById(long id)
        {
            return Context.MobileFootMenuInfo.FirstOrDefault(s => s.Id == id);
        }
        /// <summary>
        /// 修改导航栏
        /// </summary>
        ...
        {
            var sql = "UPDATE Himall_MobileFootMenus SET Name=@p1,Url=@p2,MenuIcon=@p3 WHERE Id=@p0";
            this.Context.Database.ExecuteSqlCommand(sql,footmenu.Id,footmenu.Name,footmenu.Url,footmenu.MenuIcon);
        }
        /// <summary>
        /// 增加导航栏
        /// </summary>
        ...
        public void AddMobileFootMenu(MobileFootMenuInfo footmenu)
        {
            using (DbConnection connection = new MySqlConnection(Connection.ConnectionString))
            {
                ...
                dbCmd.CommandText =
                          "INSERT INTO Himall_MobileFootMenus  (Name, Url, MenuIcon)" +
                          "VALUES (@Name, @Url, @MenuIcon)";
                dbCmd.Parameters.Add(new MySqlParameter("@Name", footmenu.Name));
                dbCmd.Parameters.Add(new MySqlParameter("@Url", footmenu.Url));
                dbCmd.Parameters.Add(new MySqlParameter("@MenuIcon", footmenu.MenuIcon));
        ...
        {
            var sql = "Delete From Himall_MobileFootMenus WHERE Id=@p0 ";
