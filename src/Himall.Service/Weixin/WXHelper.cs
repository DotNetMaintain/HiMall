        ...
        public string GetAccessToken(string appid, string secret, bool IsGetNew = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(appid) && !string.IsNullOrWhiteSpace(secret))
            {
                try
                {
                    if (IsGetNew)
                    {
                        result = GetResponseResult(string.Format("https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={0}&secret={1}", "wx8b2369d7a650c5eb", "727336d57bd539781ac341124192afa7"));
            ...
            return resultObj["access_token"].ToString();
        }
        /// <summary>
        /// 获取微信票据
        /// </summary>
