...
namespace Himall.Service
{
    public class MemberService : ServiceBase, IMemberService
    {
        ...
        public UserMemberInfo GetMemberByphone(string MemberPhone)
        {
            UserMemberInfo result = Context.UserMemberInfo.FirstOrDefault(d => d.CellPhone == MemberPhone);
            return result;
        }
        ...
        public List<UserMemberInfo> GetMembersByName(bool status, string userName)
        {
            return Context.UserMemberInfo.Where(d => d.ParentSellerId == 0 && d.Disabled == status && d.UserName == userName).ToList();
        }
        ...
        {
            var m = Context.UserMemberInfo.FirstOrDefault(p => p.Id == model.Id);
            m.Nick = model.Nick;
            m.Email = model.Email;
        ...
        {
            Context.UserMemberInfo.Remove(n => n.Id == id);
        ...
        public QueryPageModel<UserMemberInfo> GetMembers(MemberQuery query)
        {
            ...
            {
                users = users.Where(d => d.UserName.Contains(query.keyWords));
            }
            ...
            #endregion
            users = users.GetPage(d => d.OrderBy(o => o.Id), out total, query.PageNo, query.PageSize);
        ...
        {
            List<UserMemberInfo> result = new List<UserMemberInfo>();
            if (userIds.Count() > 0)
            {
                var sql = new Sql("SELECT * FROM Himall_Members WHERE Id in (@0)", userIds);

                result = DbFactory.Default.Query<UserMemberInfo>(sql).ToList();
            }
            return result;
        }
        ...
        {
            var sql = this.Context.MemberOpenIdInfo.Where(p => p.UserId == userId && p.AppIdType == appIdType);
        ...
        {
            Context.UserMemberInfo.Remove(n => ids.Contains(n.Id));
            Context.SaveChanges();
        ...
        {
            var values = Context.SiteSettingsInfo.FindAll().ToList();
            if (values != null && values.Count() > 0)
            {
                var isOpenPC = values.FirstOrDefault(a => a.Key == "IsOpenPC").Value.ToLower() == "true";
                var isOpenH5 = values.FirstOrDefault(a => a.Key == "IsOpenH5").Value.ToLower() == "true";
                var isOpenMallSmallProg = values.FirstOrDefault(a => a.Key == "IsOpenMallSmallProg").Value.ToLower() == "true";
                var isOpenApp = values.FirstOrDefault(a => a.Key == "IsOpenApp").Value.ToLower() == "true";
                var isOpenMultiStoreSmallProg = values.FirstOrDefault(a => a.Key == "IsOpenMultiStoreSmallProg").Value.ToLower() == "true";
                if (!(isOpenPC || isOpenH5 || isOpenMallSmallProg||isOpenApp||isOpenMultiStoreSmallProg))//授权模块影响会员折扣功能
                {
                    return 1;
                }
            }
        ...
        public UserCenterModel GetUserCenterModel(long id)
        {
            ...
            //  model.Expenditure = context.UserMemberInfo.FirstOrDefault(p => p.Id == id).Expenditure;
            ...
            model.RefundCount = Context.OrderRefundInfo.Count(a => a.UserId == id && (a.ManagerConfirmStatus != OrderRefundInfo.OrderRefundConfirmStatus.Confirmed));
            model.WaitPayOrders = orders.Where(a => a.OrderStatus == OrderInfo.OrderOperateStatus.WaitPay).Count();
            model.WaitReceivingOrders = orders.Where(a => a.OrderStatus == OrderInfo.OrderOperateStatus.WaitReceiving || a.OrderStatus == OrderInfo.OrderOperateStatus.WaitSelfPickUp).Count();
        ...
        public UserMemberInfo Register(string username, string password, string serviceProvider, string openId, string sex = null, string headImage = null, long introducer = 0, string nickname = null, string unionid = null, string city = null, string province = null)
        {
            ...
            //检查输入合法性
            if (string.IsNullOrWhiteSpace(username))
                throw new ArgumentNullException("电话号码不能为空");
            if (CheckMemberExist(username))
                throw new HimallException("电话号码 " + username + " 已经被其它会员注册");
        ...
        public UserMemberInfo QuickRegisters(string phone, string username, string realName, string nickName, string serviceProvider, string openId, string unionid, string sex = null, string headImage = null, MemberOpenIdInfo.AppIdTypeEnum appidtype = MemberOpenIdInfo.AppIdTypeEnum.Normal, string unionopenid = null, string city = null, string province = null)
        {
            //if (string.IsNullOrEmpty(unionid) && string.IsNullOrEmpty(openId))
            //    throw new ArgumentNullException("unionid and openId");
            ...
            if (userMember == null)
            {
                ...
                Core.Log.Debug("username结果" + username);
                Core.Log.Debug("serviceProvider结果" + serviceProvider);
                //if (string.IsNullOrWhiteSpace(username))
                //    throw new ArgumentNullException("用户名不能为空");
                //if (string.IsNullOrWhiteSpace(serviceProvider))
                //    throw new ArgumentNullException("服务提供商不能为空");
        ...
        public UserMemberInfo GetMemberByUnionId(string serviceProvider, string OpenId)
        {
            UserMemberInfo memberInfo = null;
        ...
        public UserMemberInfo GetMemberByCellphone(string phone)
        {
            UserMemberInfo memberInfo = null;
            if (!string.IsNullOrWhiteSpace(phone))
            {
                var memberOpenInfo = Context.MemberOpenIdInfo.FirstOrDefault(item => item.MemberInfo.CellPhone == phone);
                if (memberOpenInfo != null)
                    memberInfo = Context.UserMemberInfo.FirstOrDefault(p => p.Id == memberOpenInfo.UserId);
            }
            return memberInfo;
        ...
        public UserMemberInfo GetMemberByUnionIdOpenId(string UnionId, string openId)
        {
            ...
            {
                var memberOpenInfo = Context.MemberOpenIdInfo.FirstOrDefault(item => item.UnionId == UnionId && item.OpenId == openId);
                if (memberOpenInfo != null)
                    memberInfo = Context.UserMemberInfo.FirstOrDefault(p => p.Id == memberOpenInfo.UserId);
            }
            return memberInfo;
        ...
        public QueryPageModel<UserMemberInfo> GetPurchasingPowerMember(MemberPowerQuery query)
        {
            ...
            int total = 0;
            result = result.GetPage(orderBy, out total, query.PageNo, query.PageSize);
        ...
        public void DecreaseMemberOrderNumber(long userId)
        {
            ...
            //修正负数
            sql = "UPDATE Himall_Members SET OrderNumber=0 WHERE OrderNumber<0";
            this.Context.Database.ExecuteSqlCommand(sql, userId);
        }
