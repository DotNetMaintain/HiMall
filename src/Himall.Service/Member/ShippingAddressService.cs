...
namespace Himall.Service
{
    public class ShippingAddressService : ServiceBase, IShippingAddressService
    {
        ...
        public IQueryable<ShippingAddressInfo> GetUserShippingAddressByUserId(long userId)
        {
            ...
            var siteSettingService = ServiceProvider.Instance<ISiteSettingService>.Create;
            ...
            foreach (var m in model)
            {
                ...
                //m.NeedUpdate = ((m.Latitude.HasValue ? m.Latitude <= 0 : true) || (m.Longitude.HasValue ? m.Longitude <= 0 : true))
                //    && siteSettingService.GetSiteSettings().IsOpenStore;
                m.NeedUpdate = false;
            }
            return model;
        }
        ...
        public ShippingAddressInfo GetUserShippingAddress(long shippingAddressId)
        {
            ...
            var regionService = ServiceProvider.Instance<IRegionService>.Create;
            var siteSettingService = ServiceProvider.Instance<ISiteSettingService>.Create;
            var address = NoLazyNoProxyContext.ShippingAddressInfo.Find(shippingAddressId);
            if (address == null)
            {
            ...
            address.NeedUpdate = ((address.Latitude.HasValue ? address.Latitude <= 0 : true) ||
                (address.Longitude.HasValue ? address.Longitude <= 0 : true))
                && siteSettingService.GetSiteSettings().IsOpenStore;
        ...
        public ShippingAddressInfo GetDefaultUserShippingAddressByUserId(long userId)
        {
            ...
            if (defaultShippingAddressInfo != null)
            {
                ...
                defaultShippingAddressInfo.NeedUpdate = ((defaultShippingAddressInfo.Latitude.HasValue ? defaultShippingAddressInfo.Latitude <= 0 : true) ||
                    (defaultShippingAddressInfo.Longitude.HasValue ? defaultShippingAddressInfo.Longitude <= 0 : true))
                    && siteSettingService.GetSiteSettings().IsOpenStore;
            }
