...
namespace Himall.Service
{
    public class MemberIntegralService : ServiceBase, IMemberIntegralService
    {
        ...
        public void AddMemberIntegralNotAddHistoryIntegrals(MemberIntegralRecord model, IConversionMemberIntegralBase conversionMemberIntegralEntity = null)
        {
            if (null == model) { throw new NullReferenceException("添加会员积分记录时，会员积分Model为空."); }
            if (0 == model.MemberId) { throw new NullReferenceException("添加会员积分记录时，会员Id为空."); }
            if (!Context.UserMemberInfo.Any(a => a.Id == model.MemberId && a.UserName == model.UserName))
        ...
        public QueryPageModel<MemberIntegral> GetMemberIntegralList(IntegralQuery query)
        {
            int total = 0;
            IQueryable<MemberIntegral> list = Context.MemberIntegral.AsQueryable();
            if (!string.IsNullOrWhiteSpace(query.UserName))
            {
                list = list.Where(item => item.UserName.Contains(query.UserName));
            }
            ...
            list = list.GetPage(d => d.OrderByDescending(item => item.HistoryIntegrals), out total, query.PageNo, query.PageSize);
            QueryPageModel<MemberIntegral> pageModel = new QueryPageModel<MemberIntegral>() { Models = list.ToList(), Total = total };
            return pageModel;
        }

        public QueryPageModel<MemberIntegralRecord> GetIntegralRecordList(IntegralRecordQuery query)
        {
            int total = 0;
            ...
            QueryPageModel<MemberIntegralRecord> pageModel = new QueryPageModel<MemberIntegralRecord>() { Models = list.ToList(), Total = total };
            return pageModel;
        }

        public QueryPageModel<MemberIntegralRecord> GetIntegralRecordListForWeb(IntegralRecordQuery query)
        {
            int total = 0;
            IQueryable<MemberIntegralRecord> list = Context.MemberIntegralRecord.AsQueryable();
            if (query.UserId.HasValue)
