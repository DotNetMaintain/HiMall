...
namespace Himall.Service
{
    public class MemberInviteService : ServiceBase, IMemberInviteService
    {
        ...
        {
            int total = 0;
            QueryPageModel<InviteRecordInfo> result = new QueryPageModel<InviteRecordInfo>();
            var datasql = Context.InviteRecordInfo.AsQueryable();
            if (!string.IsNullOrEmpty(query.userName))
            {
                datasql = datasql.Where(d => d.UserName.Equals(query.userName));
            }
            datasql = datasql.GetPage(d => d.OrderByDescending(a => a.Id), out total, query.PageNo, query.PageSize);
            //数据转换
