...
namespace Himall.Service
{
    public class ManagerService : ServiceBase, IManagerService
    {
        ...
        {
            int total = 0;
            IQueryable<ManagerInfo> users = Context.ManagerInfo.FindBy(item => item.ShopId == query.ShopID && item.RoleId != 0 && item.Id != query.userID, item => item.Id, query.PageNo, query.PageSize, out total);
            QueryPageModel<ManagerInfo> pageModel = new QueryPageModel<ManagerInfo>()
            {
                Models = users.ToList(),
                Total = total
        ...
        public MyMemberOpenId GetByOpenId(string openid)
        {
            MemberOpenIdInfo memberOpenIdInfo = Context.MemberOpenIdInfo.FirstOrDefault(a => a.OpenId == openid);
            MyMemberOpenId myMemberOpenId = new MyMemberOpenId();
            if (memberOpenIdInfo == null)
            {
                myMemberOpenId.UserId = 0;
            }
            else
            {
                myMemberOpenId.UserId = memberOpenIdInfo.UserId;
            }
            return myMemberOpenId;
        }
        public MyMemberOpenId getOpenIdByUserId(long userid)
        {
            MemberOpenIdInfo memberOpenIdInfo = Context.MemberOpenIdInfo.FirstOrDefault(a => a.UserId == userid);
            MyMemberOpenId myMemberOpenId = new MyMemberOpenId();
            if (memberOpenIdInfo == null)
            {
                myMemberOpenId.OpenId = "0";
            }
            else
            {
                myMemberOpenId.OpenId = memberOpenIdInfo.OpenId;
            }
            return myMemberOpenId;
        }
        public UserMember GetMemberByOpenID(string openid)
        {
            var sql = "select m.id,m.Nick from himall_members m inner join himall_memberOpenIds mo on m.id=mo.userid where mo.openid='" + openid+"'";

            UserMember userMember = Context.Database.SqlQuery<UserMember>(sql).FirstOrDefault();
            return userMember;
        }
        public UserMember GetMemberByUserid(long userId)
        {
            var sql = "select Nick from himall_members where id=" + userId;

            UserMember userMember = Context.Database.SqlQuery<UserMember>(sql).FirstOrDefault();
            return userMember;
        }
        public MyOrder GetOrderByUserIdAndOrderStatus(long userId)
        {
            var sql = "select OrderStatus from himall_orders where userId=" + userId;

            MyOrder orderInfo = Context.Database.SqlQuery<MyOrder>(sql).FirstOrDefault(); 
            return orderInfo;
        }
