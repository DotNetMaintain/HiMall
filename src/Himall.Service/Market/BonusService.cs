...
namespace Himall.Service
{
    public class BonusService : ServiceBase, IBonusService
    {
        ...
        {
            BonusInfo info = Context.BonusInfo.FirstOrDefault(p => p.Id == model.Id);
        ...
        {
            BonusInfo info = Context.BonusInfo.FirstOrDefault(p => p.Id == id);
            info.ReceiveCount++;
            Context.Configuration.ValidateOnSaveEnabled = false;
            Context.SaveChanges();
            Context.Configuration.ValidateOnSaveEnabled = true;
        ...
        public QueryPageModel<BonusInfo> Get(int type, int state = 1, string name = "", int pageIndex = 1, int pageSize = 20)
        {
            ...
            QueryPageModel<BonusInfo> pageModel = new QueryPageModel<BonusInfo>()
            {
                Models = datas.ToList(),
                Total = total
            };
        ...
        public QueryPageModel<BonusReceiveInfo> GetDetail(long bonusId, int pageIndex = 1, int pageSize = 15)
        {
            ...
            QueryPageModel<BonusReceiveInfo> pageModel = new QueryPageModel<BonusReceiveInfo>()
            {
                Models = datas.ToList(),
                Total = total
            };
        ...
        public void GetSuccessSendWXMessage(BonusReceiveInfo data, string openId)
        {
            ...
            msgdata.keyword3.value = data.Himall_Bonus.EndTime.ToString("yyyy-MM-dd HH:mm");
        ...
        {
            Himall.Entity.Entities efContext = new Entity.Entities();
        ...
        public void DepositToRegister(long userid)
        {
            ...
            Core.Log.Debug("openInfos结果" + openInfos);
        ...
        //平台红包存储
        private void DepositToRegister(MemberOpenIdInfo openInfo, Himall.Entity.Entities efContext)
        {
            //获取某个OpenId对应的红包记录
        ...
        //商家红包存储
        private void DepositShopBonus(MemberOpenIdInfo openInfo, Himall.Entity.Entities efContext)
        {
            ...
            {
                Core.Log.Debug("Message结果2" + e.Message);
                Log.Info("商家红包存储出错：", e);
            }
        }
        #endregion
        ...
        {
            return Context.BonusInfo.FirstOrDefault(p => p.Id == id);
        }
