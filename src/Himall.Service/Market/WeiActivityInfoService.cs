...
namespace Himall.Service
{
    public class ActivityInfoService : ServiceBase, IWeiActivityInfoService
    {
        ...
        public QueryPageModel<WeiActivityInfo> Get(string text, WeiActivityType type, int pageIndex, int pageSize, bool? isIntegralActivity = null, bool isShowAll = true)
        {
            ...
            QueryPageModel<WeiActivityInfo> pageModel = new QueryPageModel<WeiActivityInfo>()
            {
                Models = datas.ToList(),
                Total = total
            };
        ...
        public WeiActivityInfo GetActivityModel(long id, string url = "")
        {
            try
            {
                ...
                {
                    Himall.Core.HimallIO.DeleteFile(weiInfo.CodeUrl);
        ...
        {
            //删除主表信息
            var ociobj = Context.WeiActivityInfo.FirstOrDefault(p => p.Id == id);
            if (ociobj != null)
            {
                //删除相关信息
            ...
            ////删除主表信息
            //var activityInfo = context.WeiActivityInfo.FirstOrDefault(p => p.Id == id);
