...
namespace Himall.Service
{
    public class CollocationService : ServiceBase, ICollocationService
    {
        ...
        {
            int total = 0;
            var coll = Context.CollocationInfo.Join(Context.ShopInfo, a => a.ShopId, b => b.Id, (a, b) => new
            {
                Id = a.Id,
            ...
            var collList = coll.Join(Context.CollocationPoruductInfo.Where(t => t.IsMain == true), a => a.Id, b => b.ColloId, (a, b) => new
            {
                Id = a.Id,
            ...
            {
                collList = collList.Where(d => d.Title.Contains(query.Title));
            }
            if (query.ShopId.HasValue)
            {
                collList = collList.Where(d => d.ShopId == query.ShopId.Value);
            }
            if (query.Status > 0)
            {
                switch (query.Status)
                {
                    case 1:
                        collList = collList.Where(d => d.StartTime <= DateTime.Now && d.EndTime >= DateTime.Now); break;//进行中
                    case 2:
                        collList = collList.Where(d => d.StartTime > DateTime.Now); break;//未开始
                    case 3:
                        collList = collList.Where(d => d.EndTime < DateTime.Now); break;//已结束

                }
            }
            var ddd = collList.GetPage(d => d.OrderByDescending(item => item.StartTime), out total, query.PageNo, query.PageSize);//
            var datalist = ddd.ToList();
            QueryPageModel<CollocationInfo> pageModel = new QueryPageModel<CollocationInfo>()
            {
                Models = datalist.Select(a => new CollocationInfo
                {
                    Id = a.Id,
                    CreateTime = a.CreateTime,
                    StartTime = a.StartTime,
