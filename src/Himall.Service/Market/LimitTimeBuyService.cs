...
namespace Himall.Service
{
    ...
    public class LimitTimeBuyService : ServiceBase, ILimitTimeBuyService
    {
        ...
        {
            var market = Context.LimitTimeMarketInfo.FirstOrDefault(p => p.Id == Id);
        ...
        /// <returns></returns>
        public QueryPageModel<ActiveMarketServiceInfo> GetBoughtShopList(MarketBoughtQuery query)
        {
            ...
            QueryPageModel<ActiveMarketServiceInfo> pageModel = new QueryPageModel<ActiveMarketServiceInfo>() { Models = markets.ToList(), Total = total };
            return pageModel;
        }
        ...
        /// <returns></returns>
        public QueryPageModel<LimitTimeMarketInfo> GetItemList(LimitTimeQuery query)
        {
            ...
            //
            QueryPageModel<LimitTimeMarketInfo> pageModel = new QueryPageModel<LimitTimeMarketInfo>() { Models = limit.ToList(), Total = total };
            return pageModel;
        }
        ...
        {
            CheckLimit(model);
            var item = Context.LimitTimeMarketInfo.FirstOrDefault(p => p.Id == model.Id);
        ...
        public LimitTimeMarketInfo GetLimitTimeMarketItem(long id)
        {
            ...
            var item = Context.LimitTimeMarketInfo.FirstOrDefault(p => p.Id == id);
            return item;
        ...
        public FlashSaleInfo GetLimitTimeMarketItemByProductId(long pid)
        {
            ...
            //var sql = new Sql("SELECT * FROM himall_flashsale WHERE ProductId=@0 AND Status=@1 AND BeginDate<=@2 AND EndDate > @3", pid, FlashSaleInfo.FlashSaleStatus.Ongoing.GetHashCode(), now, now);
            result = Context.FlashSaleInfo.Where(e => e.ProductId == pid && e.Status == FlashSaleInfo.FlashSaleStatus.Ongoing && e.BeginDate <= now && e.EndDate > now).FirstOrDefault();
            //在一个事务中，不能使用多个连接
        ...
        public QueryPageModel<FlashSaleInfo> GetAll(FlashSaleQuery query)
        {
            ...
            QueryPageModel<FlashSaleInfo> pageModel = new QueryPageModel<FlashSaleInfo>() { Models = limit.ToList(), Total = total };
            return pageModel;
        ...
        public QueryPageModel<FlashSaleInfo> GetStartData(int index, int size, string cname)
        {
            ExpiredDataChangeStatus();
        ...
        QueryPageModel<FlashSaleInfo> ILimitTimeBuyService.GetAll(int status, string shopname, string title, int pageIndex, int pageSize, string productName)
        {
            ExpiredDataChangeStatus();
            ...
            QueryPageModel<FlashSaleInfo> pageModel = new QueryPageModel<FlashSaleInfo>()
            {
                Models = datas.ToList(),
                Total = total
            };
        ...
        public QueryPageModel<FlashSaleInfo> GetAll(long shopid, int? status, string productName, DateTime? StartDate, DateTime? EndDate, int pageIndex, int pageSize)
        {
            ExpiredDataChangeStatus();
        ...
        {
            var product = Context.ProductInfo.FirstOrDefault(p => p.Id == productId);
        ...
        public FlashSaleModel Get(long id)
        {
            ...
            result.Quantity = skus.Sum(a => a.Stock);
        ...
        public void Pass(long id)
        {
            ...
            var detail = Context.FlashSaleDetailInfo.FirstOrDefault(p => p.FlashSaleId == model.Id && p.ProductId == model.ProductId);
        ...
        public void IncreaseSaleCount(List<long> orderids)
        {
            ...
            var now = DateTime.Now;
            var result = Context.FlashSaleInfo.Where(e => e.ProductId == productid && e.Status == FlashSaleInfo.FlashSaleStatus.Ongoing && e.BeginDate <= now && e.EndDate > now).FirstOrDefault(); ;
            if (result != null)
            {
                ...
                Context.SaveChanges();
                //var sql = new Sql("update Himall_FlashSale set SaleCount =@0  where id=@1", result.SaleCount, result.Id);
                //Context.Database.ExecuteSqlCommand(sql.SQL);
        ...
        public void ReduceSaleCount(OrderRefundInfo refund)
        {
            ...
            long productid = refund.OrderItemInfo.ProductId;
            ...
            if (result != null)
            {
                var refunds = Context.OrderRefundInfo.Where(p => p.OrderId == orderid);
                if (refunds.Any())
                {
                    var refoundInfo = refunds.FirstOrDefault();                    
                    if (refoundInfo != null)
                    {
                        long saleCount = 0;
                        //退款取订单详情数量
                        ...
                        else if (refoundInfo.RefundMode == OrderRefundInfo.OrderRefundMode.ReturnGoodsRefund)
                        {
                            if (refund.IsReturn)
                            {
                                ...
                                if (!string.IsNullOrEmpty(refoundInfo.ExpressCompanyName) && !string.IsNullOrEmpty(refoundInfo.ShipOrderNumber))
                                    saleCount = refoundInfo.ReturnQuantity.HasValue ? refoundInfo.ReturnQuantity.Value : 0;
                            }
                        }
                        //弃货不扣销量
        ...
        public List<FlashSalePrice> GetPriceByProducrIds(List<long> ids)
        {
            ...
            var sql = new Sql("SELECT ProductId,MinPrice FROM HiMall_FlashSale ");
        ...
        {
            var data = Context.FlashSaleInfo.FirstOrDefault(d => d.Id == id);
            if (data != null)
            {
        ...
        private void SendRemind(Dictionary<string, object> args)
        {
            ...
            try
            {
                Himall.Entity.Entities contextEntity = new Entity.Entities();
                ...
                foreach (var remind in flashSaleReminds)
                {
                    ...
                    var msgdata = new WX_MsgTemplateSendDataModel();
                    msgdata.first.value = "您预约的商品（"+ flashSale.Himall_Products.ProductName + ")已经开始，赶快瞧瞧吧！";
                    msgdata.first.color = "#000000";
                    msgdata.keyword1.value = flashSale.BeginDate.ToString("yyyy-MM-dd HH:mm");
                    msgdata.keyword1.color = "#FF0000";
                    msgdata.keyword2.value = flashSale.EndDate.ToString("yyyy-MM-dd HH:mm");
                    msgdata.keyword2.color = "#000000";
                    msgdata.remark.value = "数量有限，先到先得，买到即赚到。";
                    msgdata.remark.color = "#000000";
                    ...
                    Task.Factory.StartNew(() => ServiceProvider.Instance<IWXMsgTemplateService>.Create.SendMessageByTemplate(MessageTypeEnum.LimitTimeBuy, 0, msgdata, url, openid));
                }
            }
            catch (Exception e)
            {
        ...
        {
            int count = 0;
            var sql = new Sql("select a.Quantity,b.OrderDate from himall_orderitems a inner join himall_orders b on a.orderid=b.id where b.UserId=@0 and a.IsLimitBuy=1 and a.ProductId=@1 and b.OrderStatus != 4", userId, pId);
            var limitsql = new Sql("select * from himall_flashsale where ProductId=@0 and Status=2 and EndDate>=@1", pId, DateTime.Now);

            var list = DbFactory.Default.Query<dynamic>(sql);//, new { UserId = userId, ProductId = pId }).ToList();
            if (list.Any())
            {
                var limit = DbFactory.Default.FirstOrDefault<FlashSaleInfo>(limitsql);//, new { ProductId = pId, EndDate = DateTime.Now }).FirstOrDefault();
                if (limit != null)
                {
        ...
        private decimal GetMinLadderPrice(long pid)
        {
            ...
            {
                var ladder = Context.ProductLadderPricesInfo.Where(p => p.ProductId == pid)
                    .OrderBy(m => m.MinBath);
                if (ladder.Any())
                    price = ladder.FirstOrDefault().Price;
