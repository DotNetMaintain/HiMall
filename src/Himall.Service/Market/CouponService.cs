...
namespace Himall.Service
{
    public class CouponService : ServiceBase, ICouponService
    {
        ...
        {
            var shopName = Context.ShopInfo.FirstOrDefault(p => p.Id == info.ShopId).ShopName;
        ...
        public QueryPageModel<CouponInfo> GetCouponList(CouponQuery query)
        {
            ...
            QueryPageModel<CouponInfo> pageModel = new QueryPageModel<CouponInfo>() { Models = coupon.ToList(), Total = total };
            return pageModel;
        }
        public IQueryable<CouponInfo> GetCouponList(long shopid)
        {
            int auditsuccess = (int)WXCardLogInfo.AuditStatusEnum.Audited;
            return Context.CouponInfo.Include("himall_couponsetting").Where(item => item.ShopId == shopid && item.WXAuditStatus == auditsuccess);
        ...
        public IQueryable<CouponInfo> GetCouponLists(long shopId)
        {
            ...
            int auditsuccess = (int)WXCardLogInfo.AuditStatusEnum.Audited;
            return Context.CouponInfo.Where(a => a.ShopId == shopId && a.EndTime >= Date && a.WXAuditStatus == auditsuccess).OrderByDescending(a => a.Price);
        }
        /// <summary>
        /// 取用户领取的所有优惠卷信息
        /// </summary>
        ...
        public List<UserCouponInfo> GetAllUserCoupon(long userid)
        {
            ...
            var list = query.Select(b => new UserCouponInfo()
            {
                ...
                UseStatus = b.CounponStatus,
                UseTime = b.UsedTime,
                UseArea = b.Himall_Coupon.UseArea,
                Remark = b.Himall_Coupon.Remark
            }).ToList();
        ...
        public QueryPageModel<CouponRecordInfo> GetCouponRecordList(CouponRecordQuery query)
        {
            ...
            switch (query.Status)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    coupons = coupons.Where(item => item.CounponStatus == CouponRecordInfo.CounponStatuses.Used || (item.CounponStatus == CouponRecordInfo.CounponStatuses.Unuse && item.Himall_Coupon.EndTime <= date));
                    break;
            }
            coupons = coupons.GetPage(d => d.OrderByDescending(o => o.Id), out total, query.PageNo, query.PageSize);
            QueryPageModel<CouponRecordInfo> pageModel = new QueryPageModel<CouponRecordInfo>() { Models = coupons.ToList(), Total = total };
            return pageModel;
        }
        ...
        public void EditCoupon(CouponInfo info)
        {
            ...
            Context.CouponProductInfo.RemoveRange(model.Himall_CouponProducts);
            ...
            model.Himall_CouponProducts = info.Himall_CouponProducts;
            //model.EndIntegralExchange = info.EndIntegralExchange;
        ...
        public QueryPageModel<CouponInfo> GetIntegralCoupons(int page, int pageSize)
        {
            ...
            int total = 0;
            sql = sql.GetPage(d => d.OrderByDescending(o => o.CreateTime), out total, page, pageSize);
            result.Models = sql.ToList();
            result.Total = total;
            return result;
        }
        ...
        {
#if DEBUG
            Core.Log.Info(cardid + "进入同步审核：" + auditstatus.ToString());
#endif
            ...
            if (coupon != null)
            {
                ...
                Context.SaveChanges();
#if DEBUG
                Core.Log.Info(cardid + "同步审核完成：" + auditstatus.ToString());
#endif
            }
        }
        ...
        public int GetUserCouponCount(long shopId)
        {
            ...
            countsql.Append(" where A.WXAuditStatus=1 and A.ShopId=@0 and A.EndTime>CURDATE() and S.PlatForm=@1 and (S.Display=1 OR S.Display is NULL) ", shopId, PlatformType.Wap.GetHashCode());
        ...
        {
            return Context.CouponProductInfo.Where(p => p.CouponId == couponId).ToList();
        }
    }
}
