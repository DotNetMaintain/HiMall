...
namespace Himall.Service
{
    public class FightGroupService : ServiceBase, IFightGroupService
    {
        ...
        public FightGroupActiveInfo GetActive(long id, bool needGetProductCommentNumber = false, bool isLoadItems = true, bool isLoadPorductInfo = true)
        {
            ...
            result = Context.FightGroupActiveInfo.FirstOrDefault(d => d.Id == id);
            if (result == null)
            {
                throw new HimallException("错误的活动编号");
            }
            ...
            int PageSize = 10)
        {
            ...
            List<FightGroupActiveInfo> datalist = sql.GetPage(order, out total, PageNo, PageSize).ToList();
            //外键值补充
            if (datalist.Count > 0)
            {
                //商家信息
             ...
             )
        {
            ...
            int total = 0;
            var pagesql = sql.GetPage(d => d.OrderByDescending(o => o.ActiveId), out total, PageNo, PageSize);
            result.Total = total;
            ...
            )
        {
            ...
            int total = 0;
            var pagesql = sql.GetPage(d => d.OrderByDescending(o => o.JoinTime), out total, PageNo, PageSize);
            result.Total = total;
            var gpids = pagesql.Select(d => d.GroupId).ToList();
        ...
        public QueryPageModel<FightGroupOrderInfo> GetFightGroupOrderByUser(int PageNo, int PageSize, long userID, List<FightGroupOrderJoinStatus> status = null)
        {
            ...
            var result = sql.GetPage(d => d.OrderByDescending(o => o.JoinTime), out total, PageNo, PageSize).ToList();
            foreach (var item in result)
            {
        ...
        /// <param name="userId"></param>
        /// <returns></returns>
        public int GetMarketSaleCountForUserId(long activeId, long userId)
        {
            ...
            var actordsql = Context.FightGroupOrderInfo.Where(d => d.ActiveId == activeId && d.OrderUserId == userId && (d.JoinStatus != jfstate && d.JoinStatus != bfstate)).Select(d => d.Quantity);
        ...
        /// <param name="userId"></param>
        /// <returns></returns>
        public Dictionary<long, int> GetMarketSaleCountForProductIdAndUserId(IEnumerable<long> productIds, long userId)
        {
            int jfstate = FightGroupOrderJoinStatus.JoinFailed.GetHashCode();  //参团失败可以再次购买
            int bfstate = FightGroupOrderJoinStatus.BuildFailed.GetHashCode();  //拼团失败可以再次购买
            var countDict = Context.FightGroupOrderInfo.Where(d => productIds.Contains(d.ProductId.Value) && d.OrderUserId == userId && (d.JoinStatus != jfstate && d.JoinStatus != bfstate)).GroupBy(e => e.ProductId.Value).ToDictionary(e => e.Key, e => (int)e.Sum(item => item.Quantity));
            return countDict;
        }
            ...
            switch (status)
            {
                ...
                case FightGroupOrderJoinStatus.JoinSuccess:
                    ...
                    else
                    {
                        ...
                        Task.Factory.StartNew(() => ServiceProvider.Instance<IWXMsgTemplateService>.Create.SendMessageByTemplate(MessageTypeEnum.FightGroupJoinSuccess, gpord.OrderUserId.Value, msgdata, url));
                        #endregion
                        ...
                        msgdata.keyword1.value = gpinfo.ProductName;
                        msgdata.keyword1.color = "#000000";
                        msgdata.keyword2.value = gpord.UserName;
                        msgdata.keyword2.color = "#000000";
                        msgdata.keyword3.value = "￥" + gpord.SalePrice.ToString("F2");
                        msgdata.keyword3.color = "#000000";
                        msgdata.keyword5.value = gpord.JoinTime.HasValue ? gpord.JoinTime.Value.ToString("yyyy-MM-dd HH:mm:ss") : string.Empty;
                        ...
                        Task.Factory.StartNew(() => ServiceProvider.Instance<IWXMsgTemplateService>.Create.SendMessageByTemplate(MessageTypeEnum.FightGroupNewJoin, gpinfo.HeadUserId.Value, msgdata, url));
                        #endregion
                    }
                    break;
            }
        ...
        public List<FightGroupPrice> GetFightGroupPrice()
        {
            ...
            var sql = new Sql("select a.ProductId,a.ActivePrice,a.ActiveId from himall_fightgroupactiveitem a left join himall_fightgroupactive b on a.ActiveId = b.Id where b.StartTime < @0 and  b.EndTime > @1", DateTime.Now, DateTime.Now);
