...
namespace Himall.Service
{
    public class DistributionService : ServiceBase, IDistributionService
    {
        ...
        public QueryPageModel<PromoterInfo> GetPromoterList(PromoterQuery query)
        {
            QueryPageModel<PromoterInfo> model = new QueryPageModel<PromoterInfo>();
            ...
            var total = 0;
            var list = sql.GetPage(orderby, out total, query.PageNo, query.PageSize);
            model.Models = list.ToList();
            model.Total = total;
            return model;
        }
        ...
        /// </summary>
        /// <param name="Id"></param>
        public void EnablePromoter(long Id)
        {
            var promoter = Context.PromoterInfo.Where(a => a.Id == Id).FirstOrDefault();
            if (promoter == null)
            {
        ...
        public QueryPageModel<ProductsDistributionModel> GetDistributionlist(DistributionQuery query)
        {
            ...
            QueryPageModel<ProductsDistributionModel> model = new QueryPageModel<ProductsDistributionModel>() { Models = sqlquery.ToList(), Total = total };
            return model;
        }
        ...
        public QueryPageModel<ProformanceModel> GetPerformanceList(ProformanceQuery query)
        {
            QueryPageModel<ProformanceModel> model = new QueryPageModel<ProformanceModel>();
            var timeQuery = "";
            ...
            sb.Append("SELECT userName,m.Id,IFNULL(gtbl1.unpaid,0) as UnPaid,IFNULL(gtbl2.paid,0) as Paid,IFNULL(gtbl3.bcount,0) as TotalNumber,IFNULL(gtbl3.bsum,0) as TotalTurnover,IFNULL(gtbl1r.UnPaidRefund,0) as UnPaidRefund,IFNULL(gtbl2r.PaidRefund,0) as PaidRefund,IFNULL(gtbl3r.TotalTurnoverRefund,0) as TotalTurnoverRefund");
            ...
            sb.Append(" LEFT JOIN(SELECT UserId, sum(Brokerage) AS UnPaidRefund	FROM(SELECT	br.Brokerage,RefundAmount,UserId,bi.CreateTime,bi.`Status`	FROM Himall_BrokerageRefund br LEFT JOIN Himall_BrokerageIncome bi on br.IncomeId=bi.Id ) as t WHERE `Status` = 0 ");
            sb.Append(timeQuery);
            sb.Append(" GROUP BY UserId) as gtbl1r on m.Id=gtbl1r.UserId");
            ...
            sb.Append(" LEFT JOIN(SELECT UserId, sum(Brokerage) AS PaidRefund	FROM(SELECT	br.Brokerage,RefundAmount,UserId,bi.CreateTime,bi.`Status`	FROM Himall_BrokerageRefund br LEFT JOIN Himall_BrokerageIncome bi on br.IncomeId=bi.Id ) as t WHERE `Status` = 1 ");
            sb.Append(timeQuery);
            sb.Append(" GROUP BY UserId) as gtbl2r on m.Id=gtbl2r.UserId");
            ...
            sb.Append(" LEFT JOIN(SELECT UserId, sum(RefundAmount) AS TotalTurnoverRefund	FROM(SELECT	br.Brokerage,RefundAmount,UserId,bi.CreateTime	FROM Himall_BrokerageRefund br LEFT JOIN Himall_BrokerageIncome bi on br.IncomeId=bi.Id ) as t WHERE  1 = 1 ");
            sb.Append(timeQuery);
            sb.Append("  GROUP BY UserId) as gtbl3r on m.Id=gtbl3r.UserId");
            ...
            {
                sb.Append(" WHERE m.UserName like @UserName ");
            }
            ...
            var args = new DbParameter[] {
                      ...
                      new MySqlParameter("UserName", "%" +query.UserName+"%" )
               };
        ...
        public QueryPageModel<UserProformanceModel> GetPerformanceDetail(UserProformanceQuery query)
        {
                ...
                UserProformanceModel
            {
                ...
                RealTotalPrice = b.RealTotalPrice - b.FullDiscount - b.CouponDiscount,
            ...
            var total = 0;
            model = model.GetPage(d => d.OrderByDescending(item => item.OrderTime), out total, query.PageNo, query.PageSize);
            var datalist = model.ToList();
            //计算退款
            var incomeids = datalist.Select(i => (long?)i.InComeId).ToList();
            var reflist = Context.BrokerageRefundInfo.Where(d => incomeids.Contains(d.IncomeId)).GroupBy(d => d.IncomeId).Select(g => new
            {
                InComeId = g.Key,
                SumRefund = g.Sum(d => d.RefundAmount),
                SumBrokerageRefund = g.Sum(d => d.Brokerage),
            });
            foreach (var item in datalist)
            {
                var _datar = reflist.FirstOrDefault(d => d.InComeId == item.InComeId);
                if (_datar != null)
                {
                    item.RealTotalPriceRefund = _datar.SumRefund ?? 0;
                    item.BrokerageRefund = _datar.SumBrokerageRefund;
                }
            }
            QueryPageModel<UserProformanceModel> pageModel = new QueryPageModel<UserProformanceModel>() { Models = datalist, Total = total };
            return pageModel;
        }
        ...
        public void UpdateShopDistributor(ShopDistributorSettingInfo model)
        {
            ...
            config.DisShopBanner = model.DisShopBanner;
            Context.SaveChanges();
        }
        /// <summary>
        /// 设置商家默认分佣比例
        ...
        public List<long> GetAllDistributionProductIds(long shopid)
        {
            ...
            result = Context.ProductBrokerageInfo
                ...
                .ToList();
            var productIdList = Context.ProductInfo
                .Where(p => result.Contains(p.Id) && p.SaleStatus == ProductInfo.ProductSaleStatus.OnSale && p.AuditStatus == ProductInfo.ProductAuditStatus.Audited && p.IsDeleted == false)
                .Select(p => p.Id)
                .ToList();
        ...
        public QueryPageModel<object> GetDistributionOrders(dynamic query)
        {
            throw new NotImplementedException();
        }
        ...
        public QueryPageModel<object> GetDistributionProductsDetail(dynamic query)
        {
            throw new NotImplementedException();
        }
        /// <summary>
        ...
        public DistributionUserPerformanceSetModel GetUserPerformance(long userId)
        {
            ...
            _tmpsb.Append(" WHERE (t0.UserId = @UserId {otherwhere} and t0.Status!=-1) ");
        ...
        public string GetSkuText(string color, string size, string version)
        {
            string skuText = color;
            if (!string.IsNullOrEmpty(skuText)) skuText += ",";
            skuText += size;
            if (!string.IsNullOrEmpty(skuText)) skuText += ",";
            skuText += version;
            if (!string.IsNullOrEmpty(skuText)) skuText = "(" + skuText + ")";
            return skuText;
        }

        public QueryPageModel<DistributionFeatModel> GetUserBillList(DistributionUserBillQuery query)
        {
            if (query == null)
            {
                throw new HimallException("错误的搜索条件");
            }
            QueryPageModel<DistributionFeatModel> result = new QueryPageModel<DistributionFeatModel>();
            ...
            //{
            //    sql = sql.Where(d => query.UserIds.Contains(d.SalesUserId));
            //}
            if (!string.IsNullOrEmpty(query.UserName))
            {
                sql = sql.Where(d => d.SalesName.Contains(query.UserName));
            }
        ...
        public QueryPageModel<object> GetUserAgentProducts(long userId)
        {
            throw new NotImplementedException();
        }
        /// <summary>
        ...
        /// <returns></returns>
        public QueryPageModel<ProductBrokerageInfo> GetDistributionProducts(ProductBrokerageQuery query)
        {
            QueryPageModel<ProductBrokerageInfo> result = new QueryPageModel<ProductBrokerageInfo>();
            ...
            sql = (from d in sql
                   join w in weeksql on d.ProductId equals w.ProductId into join2
            ...
            if (query.CategoryId.HasValue && query.CategoryId != 0)
            {
                ...
                else
                {
                    ...
                    var shopCategoryIds = Context.ShopCategoryInfo.Where(p => (p.Id == shopCategoryId || p.ParentCategoryId == shopCategoryId)).Select(p => p.Id);
                    var productIds = Context.ProductShopCategoryInfo.Where(p => shopCategoryIds.Contains(p.ShopCategoryId)).Select(p => p.ProductId);
                    sql = sql.Where(p => productIds.Contains(p.ProductId.Value));
                }
            }
            ...
            int total = 0;
            sql = sql.GetPage(orderby, out total, query.PageNo, query.PageSize);
            List<ProductBrokerageInfoDto> datalist = sql.ToList();
            List<ProductBrokerageInfo> returnsql = datalist.Select(d => new ProductBrokerageInfo
            {
                Id = d.Id,
                ProductId = d.ProductId,
                AgentNum = d.AgentNum,
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public QueryPageModel<DistributionShopModel> GetShopDistributionList(DistributionShopQuery query)
        {
            QueryPageModel<DistributionShopModel> result = new QueryPageModel<DistributionShopModel>() { Total = 0 };
            var sql = Context.ProductBrokerageInfo.Include(d => d.Himall_Products)
                .Where(d => !d.Himall_Products.IsDeleted && d.Himall_Products.SaleStatus == ProductInfo.ProductSaleStatus.OnSale
        ...
        public void ApplyForDistributor(DistributionModel model)
        {
        ...
        public Himall.CommonModel.QueryPageModel<Himall.Model.DistributionProductsInfo> GetDistributionProducts(int page, int rows, string keyWords, long? categoryId = null)
        {
            ...
            int total = 0;
            sql = sql.GetPage(orderby, out total, page, rows);
