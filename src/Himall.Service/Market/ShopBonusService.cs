...
namespace Himall.Service
{
    public class ShopBonusService : ServiceBase, IShopBonusService
    {
        ...
        public QueryPageModel<ShopBonusInfo> Get(long shopid, string name, int state, int pageIndex, int pageSize)
        {
            ...
            {
                Models = datas.OrderByDescending(p => p.BonusDateStart).ToList(),
                Total = total
            };
            return pageModel;
        }
        ...
        {
            return Context.ShopBonusInfo.FirstOrDefault(p => p.Id == id);
        }
        ...
        {
            ShopBonusInfo info = Context.ShopBonusInfo.FirstOrDefault(p => p.Id == model.Id);
        ...
        {
            var coupon = Context.ShopBonusInfo.FirstOrDefault(d => d.Id == id);
            if (coupon != null)
            {
                coupon.WXCardState = (int)auditstatus;
                Context.SaveChanges();
            }
        }
    }
