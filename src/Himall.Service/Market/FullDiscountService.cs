...
namespace Himall.Service
{
    public class FullDiscountService : ServiceBase, IFullDiscountService
    {
        ...
        {
            //UpdateData(data);
            var obj = Context.ActiveInfo.FirstOrDefault(d => d.Id == data.Id);
        ...
        public QueryPageModel<ActiveInfo> GetActives(FullDiscountActiveQuery query)
        {
            ...
            if (query.Status != null)  //有状态不过UNION all
            {
                ...
                if (sql.Count() > 0)
                {
                    ...
                    List<ActiveInfo> datalist = sql.GetPage(order, out total, query.PageNo, query.PageSize).ToList();
            ...
            , int page = 1, int pagesize = 10)
        {
            ...
            if (!actproids.Any(d => d == -1))
            {
                ...
                if (categoryId.HasValue && categoryId > 0)
                {
                    ...
                    {
                        var shopCategoryIds = Context.ShopCategoryInfo.Where(p => p.ShopId == shopId && (p.Id == _cateid || p.ParentCategoryId == _cateid)).Select(p => p.Id);
                        var productIds = Context.ProductShopCategoryInfo.Where(p => shopCategoryIds.Contains(p.ShopCategoryId)).Select(p => p.ProductId);
                        prosql = prosql.Where(p => productIds.Contains(p.Id));
                    }
                    else
                    {
                        prosql = prosql.Where(item => ("|" + item.CategoryPath + "|").Contains("|" + _cateid + "|"));
                    }
                }
                ...
                int total = 0;
                var datas = prosql.GetPage(d => d.OrderByDescending(o => o.Id), out total, page, pagesize);
                result.Total = total;
