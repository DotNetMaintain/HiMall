...
namespace Himall.Service
{
    public class StatisticsService : ServiceBase, IStatisticsService
    {
        ...
        private void InitialOrderCount(MapChartDataModel model, int year, int month)
        {
            var start = new DateTime(year, month, 1);
            var end = start.AddMonths(1);
            #region TDO:ZYF 统计下单数量，上门自提订单取门店地址
            //var data = (from t in
            ...
            //                Count = G.Count()
            //            }).ToList();
            var d = Context.OrderInfo.Where(o => o.OrderDate >= start && o.OrderDate < end).OrderBy(o => o.OrderDate);
            var dataLis=new List<int>();
            foreach(var item in d.ToList())
            {
                var i = item.TopRegionId;
                if(item.TopRegionId <=0 && item.ShopBranchId.HasValue && item.ShopBranchId.Value > 0)
                {
                    var shopBranch = Context.ShopBranchInfo.FirstOrDefault(s => s.Id == item.ShopBranchId.Value);
                    if(shopBranch != null)
                    {
                        i = Convert.ToInt32(shopBranch.AddressPath.Split(',')[0]);
                    }
                }

                if(i<=0)
                {
                    var RegionPath = ServiceProvider.Instance<IRegionService>.Create.GetRegionPath(item.RegionId);
                    if (!string.IsNullOrEmpty(RegionPath))
                    {
                        i = Convert.ToInt32(RegionPath.Split(',')[0]);
                    }
                }
                dataLis.Add(i);
            }
            #endregion
            var data = dataLis.GroupBy(o => o).Select(o => new
            {
                Name = o.Key,
                Count = o.Count()
            }).ToList();
        ...
        private void InitialOrderMoney(MapChartDataModel model, int year, int month)
        {
            ...
            #region TDO:ZYF 修改区域省份统计
            //var x = from i in Context.OrderInfo
            //        select i.OrderTotalAmount;

            var d = Context.OrderInfo.Where(o => o.OrderDate >= start && o.OrderDate < end).OrderBy(o => o.OrderDate);
            var dataLis = new List<StatisticsOrderMoney>();
            foreach (var item in d.ToList())
            {
                var i = item.TopRegionId;
                if (item.TopRegionId <= 0 && item.ShopBranchId.HasValue && item.ShopBranchId.Value > 0)
                ...
                var money = new StatisticsOrderMoney()
                {
                    TopRegionId = i,
                    OrderTotalMoney = item.ProductTotalAmount + item.Freight + item.Tax
                };
                dataLis.Add(money);
            }
            var data = dataLis.GroupBy(o => o.TopRegionId).Select(s => new
            {
                Name = s.Key,
                Money = s.Sum(g => g.OrderTotalMoney)
            }).ToList();
            //var data = (from t in
            //                (from o in Context.OrderInfo
            //                 orderby o.OrderDate ascending
        ...
        public MapChartDataModel GetAreaOrderChart(OrderDimension dimension, int year, int month)
        {
            ...
            switch (dimension)
            {
                case OrderDimension.OrderCount: InitialOrderCount(model, year, month); break;
                case OrderDimension.OrderMemberCount: InitialOrderMemberCount(model, year, month); break;
                case OrderDimension.OrderMoney: InitialOrderMoney(model, year, month); break;
            }
            return model;
        ...
        private void InitialShopChartByOrderCount(DateTime start, DateTime end, int rankSize, LineChartDataModel<int> model)
        {
            ...
            var data = (from t in
                            (from o in Context.OrderInfo
                             ...
                             where o.OrderDate >= start && o.OrderDate < end && o.OrderStatus != OrderInfo.OrderOperateStatus.WaitPay && o.OrderStatus != OrderInfo.OrderOperateStatus.Close
                             select new
                             {
        ...
        public LineChartDataModel<int> GetShopRankingChart(int year, int month, SaleDimension dimension = SaleDimension.SaleCount, int rankSize = 15)
        {
            LineChartDataModel<int> chart = new LineChartDataModel<int>()
            {
        ...
        public LineChartDataModel<int> GetShopRankingChart(int year, int month, int weekIndex, SaleDimension dimension = SaleDimension.SaleCount, int rankSize = 15)
        {
            LineChartDataModel<int> chart = new LineChartDataModel<int>()
            {
                XAxisData = new string[rankSize],
        ...
        public LineChartDataModel<int> GetShopRankingChart(DateTime day, SaleDimension dimension = SaleDimension.SaleCount, int rankSize = 15)
        {
            LineChartDataModel<int> chart = new LineChartDataModel<int>()
            {
                XAxisData = new string[rankSize],
