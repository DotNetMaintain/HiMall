    ...
    public class CartService : ServiceBase, ICartService
    {
        public void AddToCart(string skuId, int count, long memberId)
        {
            if (count != 0)
            {
                ...
                var cartItem = Context.ShoppingCartItemInfo.FirstOrDefault(item => item.UserId == memberId && item.SkuId == skuId && !item.ShopBranchId.HasValue);
        ...
        public void AddToCart(IEnumerable<ShoppingCartItem> cartItems, long memberId)
        {
            foreach (var cartItem in cartItems.ToList())
            {
                ...
                var oriCartItem = Context.ShoppingCartItemInfo.FirstOrDefault(item => item.UserId == memberId && item.SkuId == cartItem.SkuId && !item.ShopBranchId.HasValue);
        ...
        {
            Context.ShoppingCartItemInfo.Remove(item => item.UserId == memeberId && !item.ShopBranchId.HasValue);
            Context.SaveChanges();
        ...
        {
            Context.ShoppingCartItemInfo.Remove(item => item.SkuId == skuId && item.UserId == memberId && !item.ShopBranchId.HasValue);
            Context.SaveChanges();
            Cache.Remove(CacheKeyCollection.CACHE_CART(memberId));
        }
        ...
        {
            Context.ShoppingCartItemInfo.Remove(item => skuIds.Contains(item.SkuId) && item.UserId == memberId && !item.ShopBranchId.HasValue);
            Context.SaveChanges();
            Cache.Remove(CacheKeyCollection.CACHE_CART(memberId));
        }
        ...
        public ShoppingCartInfo GetCart(long memeberId)
        {
            ...
            var cartItems = Context.ShoppingCartItemInfo.Where(item => item.UserId == memeberId && !item.ShopBranchId.HasValue).ToList();
            //if (!includeShopBranch)
            //{
            //    cartItems = cartItems.Where(item => !item.ShopBranchId.HasValue);
            //}
            ...
            foreach (var item in shoppingCartInfo.Items)
            {
                ...
                {
                    var branchSku = Context.ShopBranchSkusInfo.FirstOrDefault(d => d.SkuId == item.SkuId && d.ShopBranchId == item.ShopBranchId);
                    item.Status = 1;
        ...
        {
            return Context.ShoppingCartItemInfo.Where(item => item.UserId == memberId && skuIds.Contains(item.SkuId) && !item.ShopBranchId.HasValue)
                                               .Select(item => new ShoppingCartItem()
                                               {
                                                   Id = item.Id,
                                                   SkuId = item.SkuId,
        ...
        public int GetCartProductQuantity(long memberId, long productId = 0, string skuId = "")
        {
            ...
            if (productId > 0)
            {
                var shopInfo = Context.ShoppingCartItemInfo.Where(p => p.UserId == memberId && p.ProductId == productId && !p.ShopBranchId.HasValue);
                if (shopInfo != null)
                {
            ...
            {
                var shopInfo = Context.ShoppingCartItemInfo.Where(p => p.UserId == memberId && p.SkuId == skuId && !p.ShopBranchId.HasValue);
                if (shopInfo != null)
                {
                    cartQuantity += shopInfo.Sum(d => d.Quantity);
                }
        ...
        {
            return Context.ShoppingCartItemInfo.Where(item => item.UserId == memberId && productIds.Contains(item.ProductId) && !item.ShopBranchId.HasValue)
                .Select(item => new ShoppingCartItem()
                {
                    Id = item.Id,
                    SkuId = item.SkuId,
