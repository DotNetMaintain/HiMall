...
namespace Himall.Service
{
    public class TradeCommentService : ServiceBase, ITradeCommentService
    {
        public QueryPageModel<OrderCommentInfo> GetOrderComments(OrderCommentQuery query)
        {
            ...
            int total;
            orderComments = orderComments.GetPage(d => d.OrderByDescending(o => o.Id), out total, query.PageNo, query.PageSize);

            QueryPageModel<OrderCommentInfo> pageModel = new QueryPageModel<OrderCommentInfo>() { Models = orderComments.ToList(), Total = total };
            return pageModel;
        }

        public void DeleteOrderComment(long Id)
        {
            OrderCommentInfo ociobj = Context.OrderCommentInfo.FirstOrDefault(p => p.Id == Id);
            if (ociobj != null)
            {
                //删除相关信息
