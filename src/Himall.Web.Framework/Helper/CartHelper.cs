...
namespace Himall.Web.Framework
{
    ...
    public class MobileHomeProducts
    {
        public object GetMobileHomeProducts(long shopId, PlatformType platformType, int page, int rows, string keyWords, string shopName, long? categoryId = null)
        {
            ...
            {
                var brand = BrandApplication.GetBrand(item.Himall_Products.BrandId);
                return new
                {
                    ...
                    categoryName = CategoryApplication.GetCategory(long.Parse(CategoryApplication.GetCategory(item.Himall_Products.CategoryId).Path.Split('|').Last())).Name,
                    id = item.Id,
                    productId = item.ProductId,
                    shopName = item.Himall_Products.Himall_Shops.ShopName
                };
            });
