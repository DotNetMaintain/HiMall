...
namespace Himall.Application
{
    public class WeiActivityWinApplication
    {
        ...
        {
            WeiActivityInfo model = _iActivityInfoService.GetActivityModel(winModel.activityId);
            //添加中奖信息记录
            ...
            if (winModel.integrals > 0)
            {
                ...
                info.ReMark = model.ActivityType.ToDescription()+ "消耗";
            ...
            if (activityWinInfo.IsWin)
            {
                if (winModel.awardType == WeiActivityAwardType.Integral)//积分
                {
                    ...
                    info.MemberId = activityWinInfo.UserId;
                    info.RecordDate = DateTime.Now;
                    info.ReMark = model.ActivityType.ToDescription() + "中奖";
                    info.TypeId = MemberIntegral.IntegralType.WeiActivity;
        ...
        public static QueryPageModel<WeiActivityWinModel> GetActivityWin(string text, long id, int pageIndex, int pageSize)
        {
            ...
            QueryPageModel<WeiActivityWinInfo> weiInfo = _iWeiActivityWinService.Get(text, id, pageIndex, pageSize);
            ...
            QueryPageModel<WeiActivityWinModel> t = new QueryPageModel<WeiActivityWinModel>();
            t.Models = datas;
