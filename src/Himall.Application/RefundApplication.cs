...
namespace Himall.Application
{
    public class RefundApplication
    {
        ...
        /// <param name="id">订单编号</param>
        /// <returns></returns>
        public static OrderRefundInfo GetOrderRefundById(long id)
        {
            return _iRefundService.GetOrderRefundById(id);
        }
        /// <summary>
        /// 获取一条退货记录
        /// </summary>
        ...
        {
            var data = _iRefundService.GetOrderRefund(id, userId);
            var result = data.Map<DTO.OrderRefund>();
            if (result != null)
                result.IsShopBranchOrder = data.OrderItemInfo.OrderInfo.ShopBranchId > 0;
            return result;
        }
        /// <summary>
        /// 获取一条退货记录
        /// </summary>
        /// <param name="id"></param>
        public static Orderrefunddetail GetOrderRefunds(long id)
        {
            var data = _iRefundService.Orderrefunddetail(id);
            return data;
        }
        /// <summary>
        /// 获取退款/退货列表
        ...
        public static QueryPageModel<DTO.OrderRefund> GetOrderRefunds(RefundQuery refundQuery)
        {
            ...
            var resultdata = new List<DTO.OrderRefund>();
            foreach (var item in data.Models)
            {
                var tmp = item.Map<OrderRefund>();
                tmp.IsShopBranchOrder = item.OrderItemInfo.OrderInfo.ShopBranchId > 0;
                resultdata.Add(tmp);
            }
        ...
        {
            return _iRefundService.ConfirmRefund(id, managerRemark, managerName, notifyurl,0);
        }
        /// <summary>
        /// 异步通知确认退款
        /// </summary>
