...
namespace Himall.Application
{
    public class BillingApplication
    {
        ...
        /// <returns></returns>
        public static ShopWithDrawConfirmResult ShopApplyWithDraw(long Id, Himall.CommonModel.WithdrawStaus status, string Remark, string IpAddress = "", string UserName = "", string webRoot = "")
        {
            ...
            if (status == Himall.CommonModel.WithdrawStaus.Refused)//拒绝
            {
                ...
                //操作日志
                OperationLogApplication.AddPlatformOperationLog(new LogInfo
                {
                    Date = DateTime.Now,
            ...
            else if (model.CashType == Himall.CommonModel.WithdrawType.ALipay)//支付宝
            {
                if (status != WithdrawStaus.Succeed)
                {
                    throw new HimallException("错误的操作状态！");
                }
                #region 支付宝提现
                if (model.Status == WithdrawStaus.PayPending)
                {
                    throw new HimallException("等待第三方处理中，如有误操作，请先取消后再进行付款操作！");
                }
                ...
                try
                {
                    model.Status = WithdrawStaus.PayPending;
                    model.PlatRemark = Remark;
                    model.DealTime = DateTime.Now;
                    _iBillingService.UpdateShopWithDraw(model);
                    //异步通知地址
                ...
                catch (Exception ex)
                {
                    ...
                    throw new HimallException("付款接口异常!");
                }
                #endregion
            }
            else if (model.CashType == Himall.CommonModel.WithdrawType.WeiChat)//微信
            {
                ...
                if (plugins != null)
                {
                    try
                    {
                        ...
                        _iBillingService.UpdateShopWithDraw(model);
                        //资金处理
                        UpdateAccount(model.ShopId, model.CashAmount, Himall.CommonModel.ShopAccountType.WithDraw, payresult.TradNo, "微信提现", Id);
        ...
        public static ShopWithDrawConfirmResult ShopApplyWithDrawCallBack(long Id, Himall.CommonModel.WithdrawStaus status
            , EnterprisePayNotifyInfo payresult, string Remark, string IpAddress = "", string UserName = "")
        {
            var model = _iBillingService.GetShopWithDrawInfo(Id);
            ShopWithDrawConfirmResult result = new ShopWithDrawConfirmResult { };

            if (model.CashType == Himall.CommonModel.WithdrawType.ALipay)//支付宝
            {
                #region 支付宝提现
                try
                {
                    if (status == WithdrawStaus.Succeed)
                    {
                        ...
                        UpdateAccount(model.ShopId, model.CashAmount, Himall.CommonModel.ShopAccountType.WithDraw, payresult.batch_no, "支付宝提现", Id);
        ...
        {
            var model = _iBillingService.GetShopWithDrawInfo(Id);
            if (model.Status == Himall.CommonModel.WithdrawStaus.PayPending)//拒绝
            {
