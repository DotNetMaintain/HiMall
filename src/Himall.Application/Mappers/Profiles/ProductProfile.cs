...
namespace Himall.Application.Mappers.Profiles
{
    public class ProductProfile : Profile
    {
        protected override void Configure()
        {
            ...
            CreateMap<Model.ProductCommentInfo, DTO.ProductComment>()
                .ForMember(p => p.Images, config => config.MapFrom(p => p.Himall_ProductCommentsImages))
                .ForMember(p => p.ProductName, c => c.MapFrom(p => p.ProductInfo.ProductName))
                .ForMember(p => p.SkuId, c => c.MapFrom(p => p.Himall_OrderItems.SkuId))
                .ForMember(p => p.SKU, c => c.MapFrom(p => p.Himall_OrderItems.SKU))
                ;
