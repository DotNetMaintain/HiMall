...
namespace Himall.OpenApi
{
    ...
    public class UserHelper : _BaseHelper
    {
        ...
        {
            long result = 0;
            var user = _iMemberService.Register(user_name, password, mobile, email, 0);
        ...
        #region 私有
        private user_list_model UserMemberInfoMapDTO(UserMemberInfo data)
        {
            user_list_model result = null;
            if (data != null)
            {
                result = new user_list_model();
                var ui = _iMemberIntegralService.GetMemberIntegral(data.Id);
                if (ui == null)
                {
                    ui = new MemberIntegral
                    {
                        MemberId = data.Id,
                ...
                result.sex = data.Sex != null ? data.Sex.Value.ToDescription() : "男";
