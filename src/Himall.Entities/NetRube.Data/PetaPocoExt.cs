...
namespace NetRube.Data
{
    ...
    public static class PetaPocoExt
    {
        ...
        {
            return new GetBuilder<T>(db, db.Params);
        }
        ...
        {
            return new GetBuilder<T>(db, db.Params).Where(expression);
        }
        ...
        /// <returns>查询构建器</returns>
        public static GetBuilder<T> Get<T>(this Database db, List<object> args) where T : IModel
        {
            if (null == args) args = new List<object>();
            if (null != db.Params && db.Params.Count > 0) args.Add(db.Params);
        ...
        public static bool Add<T>(this Database db, T entity) where T : IModel
        {
            ...
            return db.Insert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, entity).ToBool_();
        }
        ...
        {
            var pd = PocoData.ForType(entity.GetType());
            return (T)Convert.ChangeType(db.Insert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, entity), typeof(T));
        }
        ...
        public static bool Add<T>(this Database db, IEnumerable<T> entity)
        {
            if (null == entity || entity.Count() <= 0) return false;
            return DbFactory.Default
                .InTransaction(() =>
                {
                    var flag = true;
                    var pd = PocoData.ForType(entity.First().GetType());
                    foreach (var target in entity)
                    {
                        flag = db.Insert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, target).ToBool_();
                        if (!flag) return flag;
                    }
                    return flag;
                });
        ...
        public static bool InTransaction(this Database db, Action action, Action succedAction = null, Action failedAction = null)
        {
            ...
            {
                Himall.Core.Log.Error(ex.Message, ex);
                db.AbortTransaction();
                r = false;
        ...
        /// <returns>表示此SQL 函数的返回值类型。只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发 <see cref="NotSupportedException"/> 异常。</returns>
        /// <exception cref="NotSupportedException">只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发此异常。</exception>
        public static bool ExIn<T, T1>(this T field, List<T1> gb)
            where T : struct
            where T1 : struct
        {
            throw new NotSupportedException("不能直接调用此方法，只能在 SQL 构建器中调用。");
        ...
        /// <returns>表示此SQL 函数的返回值类型。只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发 <see cref="NotSupportedException"/> 异常。</returns>
        /// <exception cref="NotSupportedException">只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发此异常。</exception>
        public static bool ExIn<T>(this T field, List<string> gb)
            where T : class
        {
            throw new NotSupportedException("不能直接调用此方法，只能在 SQL 构建器中调用。");
        }
        ...
        /// <returns>表示此SQL 函数的返回值类型。只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发 <see cref="NotSupportedException"/> 异常。</returns>
        /// <exception cref="NotSupportedException">只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发此异常。</exception>
        public static bool ExNotIn<T, T1>(this T field, List<T1> gb)
            where T : struct
            where T1 : struct
        {
            throw new NotSupportedException("不能直接调用此方法，只能在 SQL 构建器中调用。");
        ...
        /// <returns>表示此SQL 函数的返回值类型。只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发 <see cref="NotSupportedException"/> 异常。</returns>
        /// <exception cref="NotSupportedException">只能在 SQL 构建器中调用，当直接调用此 SQL 函数时将引发此异常。</exception>
        public static T ExResolve<T>(this GetBuilder gb)
        {
            throw new NotSupportedException("不能直接调用此方法，只能在 SQL 构建器中调用。");
        }

        /// <summary>
        /// 调用 SQL IFNULL 函数(Select)
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="field">字段栏</param>
