...
namespace NetRube.Data.DatabaseTypes
{
    ...
    class OracleDatabaseType : DatabaseType
    {
        ...
        public override string BuildTopSql(int take, bool dist, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, System.Collections.Generic.List<object> args)
        {
        ...
        public override string BuildPagedSql(long skip, long take, bool dist, string selectColumns, string tableName, string joins, string where, string groupby, string having, string orderby, System.Collections.Generic.List<object> args)
        {
            if (selectColumns.StartsWith("*"))
                throw new Exception("执行分页查询时不能直接使用“*”来获取所有字段，必需加上表别名。例如：select t.* from table t order by t.id");

            // Same deal as SQL Server
            return Singleton<SqlServerDatabaseType>.Instance.BuildPagedSql(skip, take, dist, selectColumns, tableName, joins, where, groupby, having, orderby, args);
        }
        #endregion
    }
}