    ...
    public class Database : IDisposable
    {
        ...
        private void CommonConstruct()
        {
            ...
            if (_providerName != null)
                _factory = GetFactory(_providerName); //DbProviderFactories.GetFactory(_providerName);
        ...
        private DbProviderFactory GetFactory(string providerName)
        {
            switch (providerName) {
                case "MySql.Data.MySqlClient":
                    return GetFactoryBase("MySql.Data.MySqlClient.MySqlClientFactory, MySql.Data, Culture=neutral, PublicKeyToken=c5687fc88969c44d");

                default: throw new NotSupportedException("未实现"+providerName);
            }
        }

        protected DbProviderFactory GetFactoryBase(params string[] assemblyQualifiedNames)
        {
            Type ft = null;
        ...
        public virtual bool OnException(Exception x)
        {
            ...
            Himall.Core.Log.Error(x.Message, x);
            Himall.Core.Log.Info(LastCommand);
            return true;
        }
        ...
        {
            return Update(tableName, primaryKeyName, poco, primaryKeyValue, poco is IModel ? (poco as IModel).ModifiedColumns : null);
        }
        ...
        {
            return Update(tableName, primaryKeyName, poco, poco is IModel ? (poco as IModel).ModifiedColumns : null);
        }
        ...
        {
            return Update(poco, null, poco is IModel ? (poco as IModel).ModifiedColumns : null);
        }
        ...
        public bool Save(string tableName, string primaryKeyName, object poco)
        {
        ...
        public void WriteLog()
        {
            try
            {
                Himall.Core.Log.Debug("时间: {0}\t数据库类型:{1}{2}{3}{2}{2}"
