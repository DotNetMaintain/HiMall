    ...
    public class ShopBranchWebController : BaseApiController
    {
        ...
        public object GetIndexData()
        {
            ...
            return new
            {
                ...
                TopSlide = model.Where(e => e.TypeId == Himall.Model.SlideAdInfo.SlideAdType.NearShopBranchHome).ToList(), //顶部轮播图
                Menu = model.Where(e => e.TypeId == Himall.Model.SlideAdInfo.SlideAdType.NearShopBranchIcon).ToList(), //菜单图
                ...
                MiddleSlide = model.Where(e => e.TypeId == Himall.Model.SlideAdInfo.SlideAdType.NearShopBranchHome2).ToList(), //中间轮播图
            };
        }
        ...
        public object GetProductDetail(long id, long shopBranchId)
        {
            ...
            if (shopBranch == null || shopBranch.Status == ShopBranchStatus.Freeze)
            {
            ...
            var branchskuList = ShopBranchApplication.GetSkus(shopBranch.ShopId, new List<long> { shopBranch.Id });
            if (branchskuList == null || branchskuList.Count <= 0)
            {
                throw new Himall.Core.HimallException("门店商品不存在");
            }
            if (branchskuList.Count(d => d.Status == ShopBranchSkuStatus.Normal) == 0)
            {
