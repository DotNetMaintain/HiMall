...
namespace Himall.API
{
    public class CartController : BaseApiController
    {
        ...
        public object PostUpdateCartItem(CartUpdateCartItemModel value)
        {
            ...
            {
                SKUInfo skuinfo = OrderApplication.GetSkuByID(sku.skuId);
                if (skuinfo != null)
                {
                    ...
                    if (productInfo != null)
                    {
                        if (productInfo.MaxBuyCount > 0 && sku.count > productInfo.MaxBuyCount && !productInfo.IsOpenLadder)
                        {
                            ...
                            //throw new HimallException(string.Format("每个ID限购{0}件", productInfo.MaxBuyCount));
                        }
                    }
                }
                ...
                if (isOpenLadder)
                {
                    ...
                    var groupCartByProduct = skus.Items.Where(item => (!item.ShopBranchId.HasValue || item.ShopBranchId == 0)).Select(c => {
            ...
            var result = new
            {
                ...
                Url = Core.HimallIO.GetRomoteImagePath("/m-IOS/Order/SubmiteByCart"),
        ...
        private List<CartProductModel> PackageCartProducts(ShoppingCartInfo cart, decimal prodPrice, decimal discount, IProductService productService, IShopService shopService, IShopBranchService shopBranchService, IVShopService vshopService, ITypeService typeservice,bool isBranch=false)
        {
            ...
            foreach (var item in cart.Items)
            {
                ...
                ShopBranchInfo shopbranch = null;
                ShopBranchSkusInfo shopbranchsku = null;
                if (item.ShopBranchId.HasValue)
                {
                    shopbranch = shopBranchService.GetShopBranchById(item.ShopBranchId.Value);
                    shopbranchsku = shopBranchService.GetSkusByIds(item.ShopBranchId.Value, new List<string> { item.SkuId }).FirstOrDefault();
                }
                ...
                if (null != shop)
                {
                    ...
                    if (sku != null)
                    {
                        ...
                        var _tmp = new CartProductModel
                        {
                            ...
                            Status = isLimit ? 1 : (null != shopbranch ? (product.IsOpenLadder ? 1 : (shopbranchsku == null ? 1 : (shopbranchsku.Status == CommonModel.ShopBranchSkuStatus.Normal) ? (item.Quantity > shopbranchsku.Stock ? 2 : 0) : 1)) : ((product.AuditStatus == ProductInfo.ProductAuditStatus.Audited && product.SaleStatus == ProductInfo.ProductSaleStatus.OnSale) ? (0 >= sku.Stock ? 2 : 0) : 1)),//门店状态【0:正常；1：冻结；2：库存不足】
