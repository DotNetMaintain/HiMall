...
namespace Himall.API
{
    public class VShopController : BaseApiController
    {
        ...
        public object GetVShop(long id, bool sv = false)
        {
            ...
            var homeProducts = ServiceProvider.Instance<IMobileHomeProductsService>.Create.GetMobileHomePageProducts(vshop.ShopId, Himall.Core.PlatformType.WeiXin)
                .OrderBy(item => item.Sequence).ThenBy(o => o.Id).Take(8);
            ...
            foreach (var item in homeProducts.ToArray())
            {
                ...
                pitem.ImageUrl = Core.HimallIO.GetRomoteProductSizeImage(item.Himall_Products.RelativePath, 1, (int)Himall.CommonModel.ImageSize.Size_350);
                pitem.Name = item.Himall_Products.ProductName;
                pitem.MarketPrice = item.Himall_Products.MarketPrice;
                pitem.SalePrice = item.Himall_Products.MinSalePrice;
                if (item.ShopId == SelfShopId)
                    pitem.SalePrice = item.Himall_Products.MinSalePrice * discount;
