    ...
    public class FightGroupController : BaseApiController
    {
        ...
        public object GetActiveDetail(long id, long grouId = 0, bool isFirst = true, string ids = "")
        {
            ...
            if (!string.IsNullOrWhiteSpace(result.ProductDefaultImage))
            {
                ...
                for (var n = 2; n < 6; n++)
                {
                    var _imgurl = HimallIO.GetRomoteProductSizeImage(result.ProductDefaultImage, n, (int)ImageSize.Size_350);
            ...
            #endregion
            //商品描述
            var ProductDescription = ServiceApplication.Create<IProductService>().GetProductDescription((long)result.ProductId);
        ...
        public object GetFightGroupOrderByUser(int page)
        {
            ...
            foreach (var item in datalist)
            {
                ...
                _tmp.ProductDefaultImage = HimallIO.GetRomoteProductSizeImage(_tmp.ProductImgPath, 1, (int)ImageSize.Size_350);
        ...
        public object GetFightGroupOrderDetail(long Id)
        {
            ...
            var ProductDescription = ServiceApplication.Create<IProductService>().GetProductDescription((long)orderDetail.ProductId);
            if (ProductDescription == null)
            {
                throw new Himall.Core.HimallException("错误的商品编号");
            }
