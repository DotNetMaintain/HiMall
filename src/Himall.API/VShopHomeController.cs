...
namespace Himall.API
{
    public class VShopHomeController : BaseApiController
    {
        public object GetVShopHome(int pageNo, int pageSize)
        {
            ...
            if (topShop != null)
            {
                var products = ServiceProvider.Instance<IMobileHomeProductsService>.Create.GetMobileHomePageProducts(topShop.ShopId, Core.PlatformType.WeiXin).OrderBy(t => t.Sequence).ThenBy(o => o.Id).Take(2).Select(item => item.Himall_Products);
