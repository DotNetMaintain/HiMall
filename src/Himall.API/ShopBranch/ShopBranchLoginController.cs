        ...
        public object GetUser(string userName, string password)
        {
            ...
            if (!string.IsNullOrEmpty(userName) && !string.IsNullOrEmpty(password))
            {
                ...
                if (seller != null)
                {
                    ...
                    if (shop != null && shop.ShopStatus == ShopInfo.ShopAuditStatus.HasExpired)
                    ...
                    memberId = UserCookieEncryptHelper.Encrypt(seller.Id, CookieKeysCollection.USERROLE_SELLERADMIN);
                ...
                if (membershop != null && membershop.ShopStatus == ShopInfo.ShopAuditStatus.HasExpired)
                {
                    return ErrorResult("门店所属店铺已过期");
                }
                if (null != membershop && membershop.ShopStatus == ShopInfo.ShopAuditStatus.Freeze)
                    return ErrorResult("门店所属店铺已冻结");
                if (member != null)
                {
                    result = SuccessResult();
                    memberId = UserCookieEncryptHelper.Encrypt(member.Id, CookieKeysCollection.USERROLE_USER);
                    result.UserKey = memberId;
