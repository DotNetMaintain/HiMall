                ...
                $.post('../CheckBuyNumber', { id: activeid, skuId: sku, count: num }, function (result) {
                    if (result.success) {
                        ...
                        if (hasbuy + num <= maxnum) {
                            ...
                            var _url = '/common/site/pay?area=mobile&platform=Wap&controller=order&action=SubmitGroup&skuId={0}&count={1}&groupActionId={2}&returnUrl={3}'.format(sku, num, activeid, returnHref);
                            if (groupid)
                            {
                                var canjoin = false;
                                $.ajax({
