...
function userOrders(status) {
    ...
    if (status == 0 || status == 1)//全部订单或待支付订单
        location.href = '/common/site/pay?area=mobile&platform=Wap&controller=member&action=orders&orderStatus=' + status + ret;
    else
                ...
                if (!(status == 5 && item.commentCount != 0)) {
                    ...
                    $.each(item.itemInfo, function (j, orderItem) {//图片已在后台处理
                        ...
                        if (item.orderStatus == 1)
                            detailUrl = '/common/site/pay?area=mobile&platform=Wap&controller=order&action=detail&id=' + item.id;
                        else
                    ...
                    switch (item.orderStatus) {
                        ...
                        case 2:
                            ...
                            if (!isrefund) {
                                orderBtns.push('<a class="btn-cnf" href="javascript:;" onclick="RefundApply({0},{1},{2})'.format(item.id, item.OrderRefundId, item.EnabledRefundAmount) + '">退款</a>');
                            }
                        ...
                        case 5:
                            ...
                            //if (item.ShipOrderNumber != "" && item.ShipOrderNumber != null && !isrefund)
                            //    orderBtns.push('<a class="btn-cnf" href="javascript:;" onclick="RefundApply({0},{1},{2})'.format(item.id, item.OrderRefundId, item.EnabledRefundAmount) + '">退货</a>');
                    ...
                    //if ((item.CanRefund == true || item.RefundStats == 4)) {
