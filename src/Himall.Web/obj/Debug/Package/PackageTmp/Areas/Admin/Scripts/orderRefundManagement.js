                    ...
                    { field: "ShopName", title: "店铺", width: 120, align: "center" },
                    {
                        ...
                        formatter: function (value, row, index) {
                            ...
                            if (row.RefundMode == 1) {
                                ...
                                html = '<img style="margin-left:15px;" width="40" height="40" src="' + row.ThumbnailsUrl + '"/>' + '<span class="overflow-ellipsis" style="width:195px;text-align:left"><a title="' + value + '" href="/product/detail/' + row.ProductId + '" target="_blank">' + value + '</a></span>';
                            }
                            return html;
                        }
                    },
    ...
    $.dialog({
        ...
        content: ['<div class="dialog-form">',
           ...
           ' </div>',
            ...
            '<p class="only-text"><input class="cor-red" id="txtmoney" value=\"' + data.Amount +'\"/></p>',
           ' </div>',
            '<div class="form-group">',
        ...
        button: [
        {
            ...
            callback: function () {
                ...
                ConfirmRefund(data.RefundId, $('#txtRefundRemark').val(), $('#txtmoney').val());
            },
            focus: true
        }]
    });
...
function ConfirmRefund(refundId, managerRemark,returnmoney) {
    var loading = showLoading();
    $.post('./ConfirmRefund', { refundId: refundId, managerRemark: managerRemark, returnmoney: returnmoney}, function (result) {
        loading.close();
        if (result.success) {
            if (result.status == 2) {
                jumpurl = result.msg;
