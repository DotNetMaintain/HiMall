    ...
    $("#list").hiMallDatagrid({
            ...
            },
        {
            ...
            formatter: function (value, row, index) {
                ...
                if (row.Status == "已清退") {
                    html.push("<a id='link_" + id + "' onclick=\"Enable('" + id + "')\";'>恢复</a>");
                }
                html.push("</span>");
                return html.join("");
            }
...
function Disable(id) {
    $.dialog.confirm('清退后，将不再享受分销员的权益，是否继续？', function () {
...
function Enable(id) {
    $.dialog.confirm('确定要恢复此分销员的身份？ ', function () {
        $.post("./Enable", { id: id }, function (data) {
            if (data.success) {
                $.dialog.succeedTips("恢复成功！", function () { window.location = window.location; });
