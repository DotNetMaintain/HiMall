...
namespace Himall.Web.Wdgj_Api
{
    ...
    public class Api : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                ResponseError(context, 40000, "错误的接入码", "GSE.SYSTEM_ERROR", "错误的接入码：appkey");
                return;
            }
            ...
            var shopwdinfo = ShopApplication.GetshopWdgjInfoByCode(appkey);
            string appSecret = "";
            if (shopwdinfo == null)
            {
                ResponseError(context, 40000, "错误的接入码", "GSE.SYSTEM_ERROR", "错误的接入码：appkey");
        ...
        public string GetOrder(HttpContext context)
        {
            ...
            {
                case "JH_01":
                    query.Status = OrderInfo.OrderOperateStatus.WaitPay;
                    break;
                case "JH_02":
                    query.Status = OrderInfo.OrderOperateStatus.WaitDelivery;
                    break;
                case "JH_03":
                    query.Status = OrderInfo.OrderOperateStatus.WaitReceiving;
                    break;
                case "JH_04":
                    query.Status = OrderInfo.OrderOperateStatus.Finish;
                    break;
                case "JH_05":
                    query.Status = OrderInfo.OrderOperateStatus.Close;
                    break;
                case "JH_99":
            ...
            foreach (var order in orders.Models)
            {
                ...
                switch (order.OrderStatus)
                {
                    ...
                    case OrderInfo.OrderOperateStatus.Finish:
                        ...
                        itemStatus = "JH_04";
                        break;
                    case OrderInfo.OrderOperateStatus.WaitSelfPickUp:
                        tradeStatus = "JH_03";
                        tradeStatusName = "等待买家确认收货";
                        itemStatus = "JH_03";
                        break;
                ...
                sb.Append(",\"ShouldPayType\":" + "\"" + order.PaymentType + "\"");
                //订单项
                sb.Append(",\"goodinfos\":");
                sb.Append("[");
                int curitemnum = 0;
                foreach (var item in order.OrderItems)
                {
                    ...
                    sb.Append(",\"tradegoodsno\":" + "\"" + item.SkuId + "\"");
        ...
        public string CheckRefundStatus(HttpContext context)
        {
            ...
            {
                sb.Append("\"code\":" + "\"" + "40000" + "\"");
                sb.Append(",\"message\":" + "\"" + "System Error" + "\"");
        ...
        public string Send(HttpContext context)
        {
            ...
            OrderInfo order = OrderApplication.GetOrderInfo(OrderId);
        ...
        public string DownloadProduct(HttpContext context)
        {
            ...
            long? ProductId = GetJObjectValue<long?>(jo, "ProductId");
            if (ProductId.HasValue && ProductId > 0)
            {
                query.Ids = new long[] { ProductId.Value };
            }
            ...
            switch (QStatus)
            {
                ...
                case "JH_04":
                    //未通过审核
                    query.AuditStatus = new ProductInfo.ProductAuditStatus[] { ProductInfo.ProductAuditStatus.AuditFailed };
                    break;
                case "JH_05":
                    //审核中商品
                    query.AuditStatus = new ProductInfo.ProductAuditStatus[] { ProductInfo.ProductAuditStatus.WaitForAuditing };
                    break;
                case "JH_06":
                    //即将上架商品(仓库)
            ...
            var _iProductService = ObjectContainer.Current.Resolve<Himall.IServices.IProductService>();
            var _iTypeService = ObjectContainer.Current.Resolve<Himall.IServices.ITypeService>();
            var _iSKUService = ObjectContainer.Current.Resolve<Himall.IServices.ISkuService>();

            var data = _iProductService.GetProducts(query);
            StringBuilder sb = new StringBuilder();
            sb.Append("{");
            sb.Append("\"code\":" + "\"" + "10000" + "\"");
            sb.Append(",\"message\":" + "\"" + "SUCCESS" + "\"");
            ...
            sb.Append("[");
            int curitemnum = 0;
            var allskus = _iSKUService.GetByProductIds(data.Models.Select(d => d.Id));
            foreach (var item in data.Models)
            {
                ...
                sb.Append(",\"num\":" + "\"" + item.SKUInfo.Sum(d => d.Stock) + "\"");
                sb.Append(",\"pictureurl\":" + "\"" + HimallIO.GetProductSizeImage(item.ImagePath, 1, ImageSize.Size_400.GetHashCode()) + "\"");
                sb.Append(",\"whsecode\":" + "\"" + "" + "\"");
                sb.Append(",\"attrbutes\":");
                sb.Append("[");
                #region 商品属性填充
                var prodAttrs = _iProductService.GetProductAttribute(item.Id).ToList();
                    ...
                    foreach (var curattid in prodAttrids)
                    {
                        ...
                        foreach (var attrV in attr.AttributesInfo.AttributeValueInfo.ToList())
                        {
                            if (prodAttrs.Any(p => p.ValueId == attrV.Id))
                            {
                                ...
                                sb.Append("{");
                                sb.Append("\"name\":\"" + attr.AttributesInfo.Name + "\",");
                ...
                var typeInfo = _iTypeService.GetType(item.TypeId);
                ...
                foreach (var skuitem in item.SKUInfo)
                {
                    ...
                    sb.Append("\"skuOuterID\":" + "\"" + skuitem.Sku + "\"");
        ...
        public string SyncStock(HttpContext context)
        {
            ...
            var stype = CommonModel.StockOpType.Normal;
            if (SyncStockType == "JH_01")
            {
                stype = CommonModel.StockOpType.Normal;
            }
            if (SyncStockType == "JH_02")
            {
                stype = CommonModel.StockOpType.Add;
            }
            StringBuilder sb = new StringBuilder();
            if (string.IsNullOrWhiteSpace(SkuId))
            {
                ProductManagerApplication.SetProductStock(new long[] { ProductId.Value }, Quantity, stype);
                sb.Append("{");
                sb.Append("\"code\":" + "\"10000\"");
                sb.Append(",\"message\":" + "\"SUCCESS\"");
            ...
            {
                ProductManagerApplication.SetSkuStock(new string[] { SkuId }, new int[] { Quantity }, stype);
