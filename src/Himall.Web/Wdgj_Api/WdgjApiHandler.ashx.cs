...
namespace Himall.Web.Wdgj_Api
{
    ...
    public class WdgjApiHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //}
            ////根据uCode获取效验码
            //var usign = Himall.ServiceProvider.Instance<IShopService>.Create.GetshopInfoByCode(wdgj.uCode).uSign;
        ...
        public string findProduct(HttpContext context)
        {
            ...
            string code = null;
            string name = null;
            Himall.Model.ProductInfo.ProductSaleStatus status = new Himall.Model.ProductInfo.ProductSaleStatus();
            if (!string.IsNullOrWhiteSpace(GoodsType))//按类型
            {
                type = GoodsType; //1在售 2下架
                if (type.Equals("OnSale"))
                    status = Himall.Model.ProductInfo.ProductSaleStatus.OnSale;
                else if (type.Equals("InStock"))
                    status = Himall.Model.ProductInfo.ProductSaleStatus.InStock;
            }
            ...
            var products = ServiceApplication.Create<IProductService>().GetProducts(query);
            ...
            try
            {
                ...
                foreach (var item in products.Models)
                {
                    ...
                    bool hasSku = ServiceApplication.Create<IProductService>().HasSKU(item.Id);

                    int isSku = hasSku ? 1 : 0;
                    sb.Append(string.Format("<ItemID><![CDATA[{0}]]></ItemID>", pid));
                    ...
                    ProductInfo proInfo = ServiceApplication.Create<IProductService>().GetProduct(item.Id);
                    sb.Append(string.Format("<Num><![CDATA[{0}]]></Num>", proInfo.SKUInfo.Sum(d => d.Stock)));//总数       
                    sb.Append("<Items>");
                    if (hasSku)
                    {
                        foreach (var sku in proInfo.SKUInfo)
                        {
                            sb.Append("<Item>");
        ...
        public string AdjustQuantity(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    if (ServiceApplication.Create<IProductService>().UpdateStockByProId(productId, quantity))
                    {
                        ProductInfo product = ServiceApplication.Create<IProductService>().GetProduct(productId);
                        sb.Append("<Result>1</Result>");
                        if (product != null && product.SaleStatus == Himall.Model.ProductInfo.ProductSaleStatus.OnSale)
                ...
                else
                {
                    ...
                    var quantityArr = Quantity.Split(',').Select(e =>
                    {
                        int id = 0;
                    ...
                    ServiceApplication.Create<IProductService>().SetSkusStock(skuIdArr, quantityArr);
                    ProductInfo product = ServiceApplication.Create<IProductService>().GetProduct(productId);
                    if (product != null)
                    {
                        sb.Append("<Result>1</Result>");
        ...
        public string GetOrderList(HttpContext context)
        {
            ...
            {
                case "0":
                    query.Status = Himall.Model.OrderInfo.OrderOperateStatus.WaitPay;
                    break;
                case "1":
                    query.Status = Himall.Model.OrderInfo.OrderOperateStatus.WaitDelivery;
                    break;
                case "-1":
                    break;
        ...
        public string GetOrderDetail(HttpContext context)
        {
            ...
            OrderInfo order = ServiceApplication.Create<IOrderService>().GetOrder(OrderId);
            if (order == null)
            {
                sb.Append("<Result>0</Result>");
            ...
            else
            {
                ...
                {
                    case Himall.Model.OrderInfo.OrderOperateStatus.WaitPay:
                        sb.Append("<OrderStatus>WAIT_BUYER_PAY</OrderStatus>");
                        break;
                    case Himall.Model.OrderInfo.OrderOperateStatus.WaitDelivery:
                        sb.Append("<OrderStatus>WAIT_SELLER_SEND_GOODS</OrderStatus>");
                        break;
                    case Himall.Model.OrderInfo.OrderOperateStatus.WaitReceiving:
                        sb.Append("<OrderStatus>WAIT_BUYER_CONFIRM_GOODS</OrderStatus>");
                        break;
                    case Himall.Model.OrderInfo.OrderOperateStatus.Finish:
                        sb.Append("<OrderStatus>TRADE_FINISHED</OrderStatus>");
                        break;
                    case Himall.Model.OrderInfo.OrderOperateStatus.Close:
                ...
                sb.Append(string.Format("<InvoiceTitle><![CDATA[{0}]]></InvoiceTitle>", order.InvoiceTitle));
                ...
                foreach (var item in order.OrderItemInfo)
                {
                    ...
                    //if (item.Status == LineItemStatus.RefundApplied || order.OrderStatus == OrderStatus.ApplyForRefund)
                    //    sb.Append("<GoodsStatus>WAIT_SELLER_AGREE</GoodsStatus>");
                    //else if (item.Status == LineItemStatus.MerchantsAgreedForReturn)
                    //    sb.Append("<GoodsStatus>WAIT_BUYER_RETURN_GOODS</GoodsStatus>");
                    //else if (item.Status == LineItemStatus.DeliveryForReturn)
                    //    sb.Append("<GoodsStatus>WAIT_SELLER_CONFIRM_GOODS</GoodsStatus>");
                    //else if (item.Status == LineItemStatus.RefundRefused)
                    //    sb.Append("<GoodsStatus>SELLER_REFUSE_BUYER</GoodsStatus>");
                    //else if (item.Status == LineItemStatus.Refunded || order.OrderStatus == OrderStatus.Refunded)
                    //    sb.Append("<GoodsStatus>SUCCESS</GoodsStatus>");
                    //else
