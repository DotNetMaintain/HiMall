...
namespace Himall.Web.Areas.Web.Controllers
{
    public class ProductController : BaseWebController
    {
        ...
        {
            var consultations = ServiceApplication.Create<IConsultationService>().GetConsultations(pId);
        ...
        public JsonResult GetCommentByProduct(long pId, int pageNo, int commentType = 0, int pageSize = 3)
        {
            ...
            var comments = ServiceApplication.Create<ICommentService>().GetCommentsByProductId(pId);
            ...
            if (comments != null && comments.Count() > 0)
            {
                ...
                var temp = result.OrderByDescending(a => a.ReviewDate).Skip((pageNo - 1) * pageSize).Take(pageSize);
                if (temp.Count() > 0 && null == temp.First().Himall_OrderItems)
                {
                    foreach (var item in temp)
                    {
                        item.Himall_OrderItems = _iOrderService.GetOrderItem(item.SubOrderId.HasValue ? item.SubOrderId.Value : 0);
                        if (null != item.Himall_OrderItems)
                        {
                            item.Himall_OrderItems.OrderInfo = _iOrderService.GetOrder(item.Himall_OrderItems.OrderId);
                        }
                    }
                }
        ...
        public JsonResult GetShopCate(long gid)
        {
            ...
            var categories = ServiceApplication.Create<IShopCategoryService>().GetShopCategory(product.ShopId).ToList();
        ...
        public JsonResult GetProductAttr(long pid)
        {
            ...
            var prodAttrs = ServiceApplication.Create<IProductService>().GetProductAttribute(pid).ToList();
        ...
        {
            var collocation = ServiceApplication.Create<ICollocationService>().GetCollocationListByProductId(productId);
            ...
            if (collocation != null && collocation.Count > 0)
            {
                ...
                foreach (var item in collocation)
                {
                    ...
                    products = item.Himall_Collocation.Himall_CollocationPoruducts
                            ...
                            }).Where(p => p.Stock > 0).OrderByDescending(o => o.IsMain).ThenBy(a => a.DisplaySequence).ToList();
        ...
        public ActionResult Details(long id = 0, long partnerid = 0)
        {
            ...
            if (CurrentUser != null)
            {
                ...
                var orderItems = ServiceApplication.Create<IOrderService>().GetOrderItemsByOrderId(orders).OrderByDescending(c => c.OrderInfo.OrderDate);
        ...
        public ActionResult GetLoginUserInfo()
        {
            ...
            {
                isLogin = true;
                //integral = ServiceApplication.Create<IMemberIntegralService>().GetMemberIntegral(CurrentUser.Id).AvailableIntegrals;
        ...
        public ActionResult UserCoupons()
        {
            ...
            //红包
            var shopBonus = isLogin ? ServiceApplication.Create<IShopBonusService>().GetCanUseDetailByUserId(CurrentUser.Id) : new List<ShopBonusReceiveInfo>();
            ...
            //用户积分
            model.MemberIntegral = isLogin ? ServiceApplication.Create<IMemberIntegralService>().GetMemberIntegral(CurrentUser.Id).AvailableIntegrals : 0;
        ...
        {
            var stock = ServiceApplication.Create<IProductService>().GetSku(skuId).Stock;
            var product = ServiceApplication.Create<IProductService>().GetSku(skuId).ProductInfo;
            var status = 0;
        ...
        public JsonResult Browse(long? shopId, long? categoryId, int? auditStatus, string ids, int page, string keyWords, int? saleStatus, bool? isShopCategory, int rows = 10, bool isLimitTimeBuy = false)
        {
            ...
            QueryPageModel<Himall.Model.ProductInfo> productEntities = ServiceApplication.Create<IProductService>().GetProducts(queryModel);
        ...
        public ActionResult GetCollocationProducts(string productIds, string colloPids)
        {
            ...
            var product = ServiceApplication.Create<IProductService>().GetProductByIds(ids).Where(a => a.SaleStatus == ProductInfo.ProductSaleStatus.OnSale && a.AuditStatus == ProductInfo.ProductAuditStatus.Audited).ToList();
        ...
        private CollocationSkusModel GetCollocationSku(ProductInfo product, long colloId)
        {
            ...
            string sizeAlias = (typeInfo == null || string.IsNullOrEmpty(typeInfo.SizeAlias)) ? SpecificationType.Size.ToDescription() : typeInfo.SizeAlias;
            string versionAlias = (typeInfo == null || string.IsNullOrEmpty(typeInfo.VersionAlias)) ? SpecificationType.Version.ToDescription() : typeInfo.VersionAlias;
            var skus = SKUApplication.GetByProductIds(new[] { product.Id });
