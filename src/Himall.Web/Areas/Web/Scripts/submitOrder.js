            ...
            $(".list", dialog).hiMallDatagrid({
				...
				[[
					...
					},
					{
					    ...
					    formatter: function (value, row, index) {
                            return '<div style="margin-bottom:15px; font-size:15px; color:#000;">{0}{4}</div><div style="margin-bottom:10px;">自提地址：{1}</div><div style="margin-bottom:30px">联系方式：{2}</div>'
								.format(row.shopBranchName, row.addressDetail, row.contactPhone, row.id, row.enabled == false ? '<label style="color: #fff; background: #f8413d;padding: 0 10px; border-radius: 2px;margin-left:5px;">#该门店缺货</label>' : '');
					    }
					}
				]]
            });
...
function InvoiceInit() {
    ...
    $("#dvInvoice .invoice-tit-list div.invoice-item").click(function () {
    ...
    $("#btnAddInvoice").click(function () {
        ...
        $("#dvInvoice .invoice-tit-list .invoice-item input:first").focus();
    ...
    $("#dvInvoice .invoice-tit-list").on("click", ".set-tit", function (e) {
        var self = this;
            ...
            $.each(addresses, function (i, address) {
                ...
                html += '<div class="item" name="li-' + address.id + '">\
                          <label>\
                             ...
                             <span class="item-text pull-left"><b>' + address.shipTo + '</b>&nbsp; ' + address.fullRegionName + ' &nbsp; ' + address.address + ' &nbsp; ' + address.addressDetail + ' &nbsp; ' + address.phone + ' </span>&nbsp\
                          </label>\
