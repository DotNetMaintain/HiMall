...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    ...
    public class DistributionRulesController : BaseSellerController
    {
        ...
        public JsonResult GetProductList(string skey, int rows, int page, long categoryId = 0)
        {
            ...
            if (datasql.Models != null)
            {
                datalist = datasql.Models.Select(d => new DistributionProductListModel
                {
                    ...
                    CategoryName = GetProductShopCategorieName(d.ProductId.Value),//d.CategoryName,
        ...
        {
            var shopCategory = Himall.Application.ShopCategoryApplication.GetCategorysByProductId(id);

            var name = shopCategory.Select(s => {
                return string.IsNullOrEmpty(s.Name) ? null : s.Name;
            }).ToArray();
        ...
        public ActionResult AdvPageSet()
        {
            ...
            result.DistributorBanner = config.DisShopBanner;
            return View(result);
        }
        [HttpPost]
        public JsonResult AdvPageSet(ShopDistributorAdvPageSetModel model)
        {
            ...
            if (isdataok)
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (model.DistributorBanner.Contains("/temp/"))
                    {
                        string oriUrl = model.DistributorBanner;
                        oriUrl = oriUrl.Substring(oriUrl.LastIndexOf("/temp"));
                        string ext = oriUrl.Substring(oriUrl.LastIndexOf('.'));
                        string _savepath = @"/Storage/Shop/" + curshopid.ToString() + "/Distribution/";
                        string _savefile = "banner" + ext;
                        _savepath = _savepath + _savefile;
                        Core.HimallIO.CopyFile(oriUrl, _savepath, true);
                        model.DistributorBanner = _savepath;
                    }
                    else if (model.DistributorBanner.Contains("/Storage/"))
                    {
                        model.DistributorBanner = model.DistributorBanner.Substring(model.DistributorBanner.LastIndexOf("/Storage/"));
                    }
                    else
                    {
                        model.DistributorBanner = "";
                    }
                    #endregion
                    ...
                    config.DisShopBanner = model.DistributorBanner;
