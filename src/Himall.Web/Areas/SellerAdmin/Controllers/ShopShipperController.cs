...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    ...
    public class ShopShipperController : BaseSellerController
    {
        ...
        public ActionResult Add(long id = 0)
        {
            ...
            {
                string token = AccessTokenContainer.TryGetToken(CurrentSiteSetting.WeixinAppId, CurrentSiteSetting.WeixinAppSecret);
                if (string.IsNullOrWhiteSpace(token))
                {
                    token = AccessTokenContainer.TryGetToken(CurrentSiteSetting.WeixinAppId, CurrentSiteSetting.WeixinAppSecret, true);
                }
