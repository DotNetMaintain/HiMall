...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    public class ProductController : BaseSellerController
    {
        ...
        private bool CanLadder()
        {
            ...
            if (!(CurrentSiteSetting.IsOpenPC || CurrentSiteSetting.IsOpenH5 || CurrentSiteSetting.IsOpenApp || CurrentSiteSetting.IsOpenMallSmallProg))
            {
                canLadder = false;
            }
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult Create(ProductCreateModel model, HttpPostedFileBase HPVY, HttpPostedFileBase HPVO)
        {
            string ExtName = "";
            string ExtNames = "";
            ...
            if (HPVY != null)
            {
                try
                {
                    ExtName = da + HPVY.FileName;
                    string path = Server.MapPath("~/Txt/"); //存储在服务器的路径
                    if (!Directory.Exists(path)) //判断路径是否存在 不存在创建
                    {
                        Directory.CreateDirectory(path);
                    }
                    string savePath = path + ExtName;
                    HPVY.SaveAs(savePath);
                }
                catch (Exception e)
                {

                    Core.Log.Error("上传文件失败", e);
                }
            }
            if (HPVO != null)
            {
                try
                {
                    ExtNames = da + HPVO.FileName;
                    string path = Server.MapPath("~/Txt/"); //存储在服务器的路径
                    if (!Directory.Exists(path)) //判断路径是否存在 不存在创建
                    {
                        Directory.CreateDirectory(path);
                    }
                    string savePaths = path + ExtNames;
                    HPVO.SaveAs(savePaths);
                }
                catch (Exception e)
                {
            ...
            model.IsHadHPV = !string.IsNullOrEmpty(model.IsHave) && model.IsHave.Equals("on");//是否开启阶梯价
            ...
            product.HPVO = ExtNames;
            product.HPVY = ExtName;
            product.Insurery = model.Insurery;
            product.Insurero = model.Insurero;
            product.Insurancey_age = model.Insurancey_age;
            product.Insuranceo_age = model.Insuranceo_age;
            var success = false;
            try
            {
            ...
            {
                //添加商家操作日志
                ServiceApplication.Create<IOperationLogService>().AddSellerOperationLog(new LogInfo
                {
                    Date = DateTime.Now,
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult Edit(ProductCreateModel model, HttpPostedFileBase HPVY, HttpPostedFileBase HPVO,string HPVOvalue, string HPVYvalue)
        {
            string ExtName = "";
            string ExtNames = "";
            string da = DateTime.Now.ToString("yyyy-MM-ddHHmmss");
            ...
            var productInfo = productservice.GetInsurance(model.Id);
            model.MinSalePrice = model.SKUExs.Min(p => p.SalePrice);//model.SKUInfoExs一定有值,当用户没有设置规格时将会有默认值
            ...
            try
            {
                Himall.Application.ProductManagerApplication.UpdateProduct(product, model.Pics, skus, descriptionInfo, attributes, model.GoodsCategory, sellerSpecs, prices);
        ...
        {
            var auditOnOff = ServiceApplication.Create<ISiteSettingService>().GetSiteSettings().ProdutAuditOnOff;
        ...
        public JsonResult List(ProductQuery queryModel, string auditStatuses, int page, int rows)
        {
            ...
            if (queryModel.SaleStatus == ProductInfo.ProductSaleStatus.InStock)
            {
                queryModel.AuditStatus = new ProductInfo.ProductAuditStatus[] { ProductInfo.ProductAuditStatus.Audited
                    , ProductInfo.ProductAuditStatus.WaitForAuditing
                    , ProductInfo.ProductAuditStatus.UnAudit
                    , ProductInfo.ProductAuditStatus.AuditFailed };
            }
        ...
        public ActionResult ExportToExcel(long? categoryId = null, string productCode = "", string brandName = "", int? auditStatus = null, string auditStatuses = null, int? saleStatus = null, string ids = "", string keyWords = "", DateTime? startDate = null, DateTime? endDate = null)
        {
            ...
            QueryPageModel<Himall.Model.ProductInfo> productEntities = _iProductService.GetProducts(queryModel);
        ...
        {
            if (ServiceApplication.Create<IShopService>().GetShopSpaceUsage(this.CurrentSellerManager.ShopId) == -1)
            {
                message = "存储图片空间不足,不能发布商品!";
                return false;
            }

            var grade = ServiceApplication.Create<IShopService>().GetShopGrade(this.CurrentShop.GradeId);
            if (grade != null)
            {
