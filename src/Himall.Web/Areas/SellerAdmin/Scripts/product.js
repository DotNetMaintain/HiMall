    ...
    window.productInit = function () {
        ...
        $('.btn.step').click(function () {
            ...
            if ($(this).hasClass('needValid')) {
                ...
                var IsHadHPV = $('#ckhLadder').is(':checked');
                if (IsHadHPV) {
                    var HPVY = $('input[id="HPVY"]');
                    var HPVO = $('input[id="HPVO"]');
                    var Insurero = $('input[id="Insurero"]');
                    var Insurery = $('input[id="Insurery"]');
                    var Insurancey_age = $('input[id="Insurancey_age"]');
                    var Insuranceo_age = $('input[id="Insuranceo_age"]');
                    if (HPVY.val() != '请导入pdf文档或者word文档' && HPVY.val() != null && HPVY.val() != '') {
                        $('input[id="HPVYvalue"]').val(HPVY.val());
                    }
                    if (HPVO.val() != '请导入pdf文档或者word文档' && HPVO.val() != null && HPVO.val() != '') {
                        $('input[id="HPVOvalue"]').val(HPVO.val());
                    }
                    if (($('input[id="HPVYvalue"]').val() == '请导入pdf文档或者word文档' || $('input[id="HPVYvalue"]').val() == null) && ($('input[id="HPVOvalue"]').val() == '请导入pdf文档或者word文档' || $('input[id="HPVOvalue"]').val() == null)) {
                        $.dialog.alertTips('请至少导入一份导入保险知情同意书！');
                        HPVY.parent().addClass('has-error');
                        HPVY.addClass('input-validation-error');
                        return false;
                    } else if (!($('input[id="HPVYvalue"]').val() == '请导入pdf文档或者word文档' || $('input[id="HPVYvalue"]').val() == null)) {
                        if ($('input[id="HPVYvalue"]').val().indexOf('.pdf') < 0 && $('input[id="HPVYvalue"]').val().indexOf('.docx') < 0) {
                            $.dialog.alertTips('同意书必须导入pdf文档或者word文档！');
                            HPVY.parent().addClass('has-error');
                            HPVY.addClass('input-validation-error');
                            return false;
                        }
                    }
                    else if (!($('input[id="HPVOvalue"]').val() == '请导入pdf文档或者word文档' || $('input[id="HPVOvalue"]').val() == null)) {
                        if ($('input[id="HPVOvalue"]').val().indexOf('.pdf') < 0 && $('input[id="HPVOvalue"]').val().indexOf('.docx') < 0) {
                            $.dialog.alertTips('同意书必须导入pdf文档或者word文档');
                            HPVO.parent().addClass('has-error');
                            HPVO.addClass('input-validation-error');
                            return false;
                        }
                    }
                    else if ($('input[id="Insurery"]').val() == '' && $('input[id="Insurero"]').val() == '') {
                        $.dialog.alertTips('请至少填写一个保险公司！');
                        Insurery.parent().addClass('has-error');
                        Insurery.addClass('input-validation-error');
                        return false;
                    }
                    else if ($('input[id="Insurancey_age"]').val() == '' && $('input[id="Insuranceo_age"]').val() == '') {
                        $.dialog.alertTips('请至少填写一个保险保险年龄段！');
                        Insurancey_age.parent().addClass('has-error');
                        Insurancey_age.addClass('input-validation-error');
                        return false;
                    } else if (!Insurancey_age.val() == '') {
                        if (Insurancey_age.val().indexOf('-') < 0) {
                            $.dialog.alertTips('年龄段必须以‘-’分隔');
                            Insurancey_age.parent().addClass('has-error');
                            Insurancey_age.addClass('input-validation-error');
                            return false;
                        }
                    } else if (!Insuranceo_age.val() == '') {
                        if (Insuranceo_age.val().indexOf('-') < 0) {
                            $.dialog.alertTips('年龄段必须以‘-’分隔');
                            Insuranceo_age.parent().addClass('has-error');
                            Insuranceo_age.addClass('input-validation-error');
                            return false;
                        }
                    }
                    //if ((HPVY.val() == '请导入pdf文档或者word文档' || HPVY.val() == '') && ($('input[id="HPVYvalue"]').val() == '请导入pdf文档或者word文档' || $('input[id="HPVYvalue"]').val() == '')) {
                    //    HPVY.val($('input[id="HPVYvalue"]').val());
                    //    alert(HPVY.val());
                    //}
                    //if ((HPVO.val() == '请导入pdf文档或者word文档' || HPVO.val() == '') && ($('input[id="HPVOvalue"]').val() == '请导入pdf文档或者word文档' || $('input[id="HPVOvalue"]').val() == '')) {
                    //    HPVO.val($('input[id="HPVOvalue"]').val());
                    //    alert(HPVO.val());
                    //}
                    //else if (HPVY.val() != '' && HPVO.val() != '') {
                    //    $('input[id="HPVYvalue"]').val(HPVY.val());
                    //    $('input[id="HPVOvalue"]').val(HPVO.val());
                    //}
        ...
        //开启批价后显示批价设置区域
        $('#ckhLadder').change(function () {
            if (this.checked == true) {
                $('.ladders').removeClass('hidden');
            } else {
                $('.ladders').addClass('hidden');
            }
        });
