(function () {
    ...
    function initCover() {
        $('.template-node:not(.template-inited)').each(function () {
            ...
            temp = $(temp).attr('title', $this.attr('t-title'));
            //$this.removeAttr('t-title');
            var zIndex = $this.attr('z-index');
            if (zIndex)
                temp.css('z-index', zIndex);
        ...
        if (hotSearch != '' && typeof (hotSearch) != "undefined") {
            ...
            if (arrHot.length > 0) {
                ...
                for (var i = 0; i < arrHot.length; i++) {
                    ...
                    content += '<a  onclick="' + link + '"  class="' + isactive + '" url="/search/searchAd?keywords=' + arrHot[i] + '" keyword="' + arrHot[i] + '">' + arrHot[i] + '</a>';
                }
