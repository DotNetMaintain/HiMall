...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class MessageController : BaseAdminController
    {
        ...
        public ActionResult editwxtm()
        {
            ...
            List<WeiXinMsgTemplateInfo> wxtempllist = new List<WeiXinMsgTemplateInfo>();
            wxtempllist = _iWXMsgTemplateService.GetWeiXinMsgTemplateList().Where(e => (!e.UserInWxApplet.HasValue) || e.UserInWxApplet == CommonModel.WXMsgTemplateType.WeiXinShop).ToList();
        ...
        public JsonResult Send(string pluginId, string destination)
        {
            ...
            var result = messagePlugin.Biz.SendTestMessage(destination, "尊敬的用户：" + siteName+ "，长生树提醒您，您正在进行测试操作", "这是一封测试邮件");
            if (result == "发送成功！")
            {
                return Json(new { success = true });
            }
