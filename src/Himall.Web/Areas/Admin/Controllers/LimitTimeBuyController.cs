...
namespace Himall.Web.Areas.Admin.Controllers
{
    ...
    public class LimitTimeBuyController : BaseAdminController
    {
        ...
        [HttpPost]
        [UnAuthorize]
        public JsonResult List(int? status, int page, int rows, string shopName, string title, string productName = "")
        {
            if (status == null)
            {
                status = 0;
            ...
            QueryPageModel<FlashSaleInfo> result = _iLimitTimeBuyService.GetAll((int)status, shopName, title, page, rows, productName);
...

           {
               ...
               m.MarketPrice = item.Himall_Products.MarketPrice;
               m.ProductImg = Himall.Core.HimallIO.GetProductSizeImage(item.Himall_Products.ImagePath, 1, (int)ImageSize.Size_350);
               return m;
           });
        ...
        {
            ViewBag.LimitTimeBuyNeedAuditing = CurrentSiteSetting.LimitTimeBuyNeedAuditing;
        ...
        public ActionResult SetConfig(FlashSaleConfigModel data)
        {
            ...
            SiteSettingApplication.SaveSetting("LimitTimeBuyNeedAuditing", isneedaudit);
