...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class NearShopBranchController : BaseAdminController
    {
        ...
        public ActionResult NearShopBranchSetting()
        {
            ...
            //轮播图
            ViewBag.imageAds = _iSlideAdsService.GetSlidAdsOrInit(0, SlideAdInfo.SlideAdType.NearShopBranchSpecial).ToList();
            //门店授权
        ...
        public JsonResult AddSlideImage(string id, string description, string imageUrl, string url, int slideTypeId)
        {
            ...
            slideAdInfo.TypeId = (SlideAdInfo.SlideAdType)slideTypeId;
        ...
        {
            //轮播图
            var slideImageSettings = _iSlideAdsService.GetSlidAds(0, (SlideAdInfo.SlideAdType)id).ToArray();
        ...
        {
            _iSlideAdsService.UpdateWeixinSlideSequence(0, oriRowNumber, newRowNumber, (SlideAdInfo.SlideAdType)id);
        ...
        {
            //轮播图
            var slideImageSettings = _iSlideAdsService.GetSlidAds(0, SlideAdInfo.SlideAdType.NearShopBranchIcon).ToArray();
            var slideImageService = _iSlideAdsService;
            var slideModel = slideImageSettings.Select(item =>
            {
                //var slideImage = slideImageService.GetSlidAd(0, item.Id);
        ...
        public JsonResult AddNearShopBranchIcon(string id, string description, string imageUrl, string url)
        {
            ...
            slideAdInfo.Id = Convert.ToInt64(id);
            slideAdInfo.ImageUrl = imageUrl;
            slideAdInfo.TypeId = SlideAdInfo.SlideAdType.NearShopBranchIcon;
            slideAdInfo.Url = url.ToLower();
            slideAdInfo.Description = description;
            slideAdInfo.ShopId = 0;
            if (slideAdInfo.Id > 0)
        ...
        {
            _iSlideAdsService.UpdateWeixinSlideSequence(0, oriRowNumber, newRowNumber, SlideAdInfo.SlideAdType.NearShopBranchIcon);
        ...
        public JsonResult ShopTagList(int page, int rows, string titleKeyword)
        {
            ...
            //var dataGrid = new DataGridModel<ShopBranchTagModel>() { rows = shopBranchTagInfos.Skip(page * rows).Take(rows), total = shopBranchTagInfos.Count() };
            var list = new
            {
        ...
        public JsonResult UpdateImageAd(long id, string pic, string url)
        {
            ...
            var slidAd = new SlideAdInfo { ShopId = 0, Url = url, ImageUrl = pic, Id = id };
            var ret = _iSlideAdsService.UpdateSlidAd(slidAd);
            if (null != ret)
            {
                ret.ImageUrl = Core.HimallIO.GetImagePath(ret.ImageUrl);
                return Json(new Result { success = true, data = ret }, JsonRequestBehavior.AllowGet);
            }
            else
            {
                return Json(new Result { success = false, msg = "更新失败" }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public JsonResult GetRecommendShopBranch()
        {
            ...
            var shopbranchs = ShopBranchApplication.GetShopBranchsNoPager(query).OrderBy(n => n.RecommendSequence);

            return Json(new { rows = shopbranchs }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult RecommendShopBranch(long[] ids)
        {
            ...
            return Json(new Result { success = flag, msg = flag ? "" : "未知错误,请重试" }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult RecommendChangeSequence(long oriShopBranchId, long newShopBranchId)
        {
            ...
            return Json(new Result { success = flag, msg = flag ? "" : "未知错误,请重新排序" }, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var siteinfo = CurrentSiteSetting;
            switch (type)
            {
                case 1:
                    siteinfo.O2OApplet_IsUseTopSlide = status;
                    break;
                case 2:
                    siteinfo.O2OApplet_IsUseIconArea = status;
                    break;
                case 3:
                    siteinfo.O2OApplet_IsUseAdArea = status;
                    break;
                case 4:
                    siteinfo.O2OApplet_IsUseMiddleSlide = status;
                    break;
                default:
                    return Json(new Result { success = false, status = -1, msg = "错误的操作参数" });
                    break;
            }
            SiteSettingApplication.SetSiteSettings(siteinfo);
        ...
        private string GetLinkName(string url)
        {
            ...
            if (arr.Length == 2)
            {
                ...
                switch (arr[0])
                {
                    case "1":
                        {
                            var model = ShopBranchApplication.GetShopBranchTagInfo(id);
                    ...
                    case "3":
                        {
                            var model = _iTopicService.GetTopicInfo(id);
