...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class StatisticsController : BaseAdminController
    {
        ...
        public JsonResult GetWeekList(int year = 0, int month = 0)
        {
            ...
            return Json(new { success = true, week = data }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetShopRankingChart(string day = "", int year = 0, int month = 0, int weekIndex = 0, int dimension = 1)
        {
            ...
            if (!string.IsNullOrWhiteSpace(day))
            {
                ...
                model = _iStatisticsService.GetShopRankingChart(start, (SaleDimension)dimension);
            }
            else
            {
                if (year == 0) year = DateTime.Now.Year;
                if (month == 0) month = DateTime.Now.Month;
                if (weekIndex == 0)
                    model = _iStatisticsService.GetShopRankingChart(year, month, (SaleDimension)dimension);
                else
                    model = _iStatisticsService.GetShopRankingChart(year, month, weekIndex, (SaleDimension)dimension);
            }
        ...
        public ActionResult ExportSaleRanking(string day = "", int year = 0, int month = 0, int weekIndex = 0)
        {
            ...
            LineChartDataModel<int> SaleCountmodel = new LineChartDataModel<int>();
            LineChartDataModel<int> Salesmodel = new LineChartDataModel<int>();
            string Title = "";
            DateTime start;
