...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class OrderRefundController : BaseAdminController
    {
        ...
        //[HttpPost]
        public JsonResult ConfirmRefund(long refundId, string managerRemark, decimal returnmoney)
        {
            Core.Log.Debug("refundId========" + refundId + "managerRemark=====" + managerRemark + "returnmoney=====" + returnmoney);
            ...
            string refundurl = _iRefundService.ConfirmRefund(refundId, managerRemark, CurrentManager.UserName, notifyurl, returnmoney);
            ...
            Orderrefundmessage orderrefundmessage = new Orderrefundmessage();
            orderrefundmessage.Amount = returnmoney;
            orderrefundmessage.operatorname = CurrentManager.UserName;
            orderrefundmessage.RefundId = refundId;
            orderrefundmessage.remark = managerRemark;
            json.Serialize(orderrefundmessage, sb);
            string str = sb.ToString();
            string results = GetResponseResult(string.Format("http://zxk.mhybms.cn:10000/system/refund/operate_1/submitRefund?refundJson={0}", str));
            Core.Log.Debug("resultssss" + results);
            return Json(result);
        }
