...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class OrderController : BaseAdminController
    {
        ...
        public OrderController(IOrderService iOrderService, IExpressService iExpressService, IPaymentConfigService iPaymentConfigService
            ,IRefundService iRefundService, IFightGroupService iFightGroupService)
        {
        ...
        public JsonResult List(OrderQuery query, int page, int rows)
        {
            ...
            IEnumerable<OrderModel> orderModels = models.Select(item =>
            {
                ...
                return new OrderModel()
                {
                    ...
                    ShopBranchName = item.DeliveryType == CommonModel.Enum.DeliveryType.SelfTake && item.ShopBranchId.HasValue && item.ShopBranchId.Value != 0 && shopBranchs.FirstOrDefault(sb => sb.Id == item.ShopBranchId.Value) != null ? shopBranchs.FirstOrDefault(sb => sb.Id == item.ShopBranchId.Value).ShopBranchName : "",
                    UserId = item.UserId,
