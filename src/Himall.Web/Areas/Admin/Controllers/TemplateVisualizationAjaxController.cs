    ...
    public class TemplateVisualizationAjaxController : BaseAdminController
    {
        ...
        private void InitialAppHomeGiftModel(GiftAjaxModel model, int status, string name, int pageNo)
        {
            var giftList = _iGiftService.GetGifts(new GiftQuery
            {
                ...
                PageNo = pageNo,
                PageSize = 10,
                Sort = GiftQuery.GiftSortEnum.Default,
                IsAsc = true
            });
        ...
        private void InitialAppHomeGiftContentModel(IEnumerable<GiftModel> giftList, GiftAjaxModel model)
        {
            foreach (var gift in giftList)
            {
                model.list.Add(new GiftContent
                {
                    ...
                    title = gift.GiftName,
                    adddate = gift.AddDate.HasValue ? gift.AddDate.Value.ToString("yyyy-MM-dd") : "",
        ...
        public ActionResult Hi_Ajax_GetAppHomeGoodsList(int status = 2, string title = "", int p = 1)
        {
            int pageNo = p;
            ProductAjaxModel model = new ProductAjaxModel() { list = new List<ProductContent>() };
            InitialAppHomeProductModel(model, status, title, pageNo);
            return Json(model, JsonRequestBehavior.AllowGet);
        }

        private void InitialAppHomeProductModel(ProductAjaxModel model, int status, string name, int pageNo)
        {
            var productList = _iProductService.GetProducts(new ProductQuery
            {
            ...
            if (productList != null)
            {
                model.status = 1;
                model.page = TemplatePageHelper.GetPageHtml(pageCount, pageNo);
                InitialAppHomeProductContentModel(productList.Models, model);
            }
        }

        private void InitialAppHomeProductContentModel(IEnumerable<ProductInfo> productList, ProductAjaxModel model)
        {
        ...
        private void InitialBonusContentModel(IEnumerable<Himall.Model.BonusInfo> bonusList, BonusAjaxModel model)
        {
        ...
        private void InitialCouponsContentModel(IEnumerable<Himall.Model.CouponInfo> couponsList, CouponsAjaxModel model)
        {
        ...
        private void SmallProInitialCouponsContentModel(IEnumerable<Himall.Model.CouponInfo> couponsList, CouponsAjaxModel model)
        {
        ...
        private void InitialSmallProgProductContentModel(IEnumerable<ProductInfo> productList, ProductAjaxModel model)
        {
            foreach (var product in productList)
            {
                model.list.Add(new ProductContent
        ...
        private void InitialProductContentModel(IEnumerable<ProductInfo> productList, ProductAjaxModel model)
        {
            foreach (var product in productList)
            {
                model.list.Add(new ProductContent
        ...
        private void InitialLimitBuyContentModel(IEnumerable<Himall.Model.FlashSaleInfo> limitBuyList, LimitBuyAjaxModel model)
        {
        ...
        private void InitialSmallProgLimitBuyContentModel(IEnumerable<Himall.Model.FlashSaleInfo> limitBuyList, LimitBuyAjaxModel model)
        {
            var datalist = limitBuyList.ToList();
            foreach (var limitBuy in limitBuyList)
            {
        ...
        public ActionResult Hi_Ajax_SaveTemplate(int is_preview, string client, string content = "", int type = 1, long shopId = 0)
        {
            ...
            try
            {
                ...
                if (clientType == VTemplateClientTypes.WapSpecial)
                {
                    htmlsb.Append("@model Himall.Model.TopicInfo \n");
                }
                htmlsb.Append("@{\n");
                ...
                htmlsb.Append("<script src=\"https://res.wx.qq.com/open/js/jweixin-1.0.0.js\"></script>\n");
                ...
                htmlsb.Append("<a href=\"javascript:;\" class=\"btn_top\" ></a>\n");
        ...
        private void InitialPhotoSpaceModel(PhotoSpaceAjaxModel model, QueryPageModel<PhotoSpaceInfo> photo, int pageNo = 1)
        {
