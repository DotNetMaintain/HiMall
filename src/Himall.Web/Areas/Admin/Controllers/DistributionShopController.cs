...
namespace Himall.Web.Areas.Admin.Controllers
{
    ...
    public class DistributionShopController : BaseAdminController
    {
        ...
        public JsonResult ProductLists(long? categoryId, string brandName, string productCode, int? auditStatus, string ids, int page, int rows, string keyWords, string shopName, int? saleStatus)
        {
            ...
            QueryPageModel<Himall.Model.ProductBrokerageInfo> datasql = _iDistributionService.GetDistributionProducts(query);

            IEnumerable<ProductModel> products = datasql.Models.ToArray().Select(item =>
            {
                var shop = ShopApplication.GetShop(item.Product.ShopId);
                return new ProductModel()
                {
                    ...
                    shopName = shop != null ? shop.ShopName : "",
