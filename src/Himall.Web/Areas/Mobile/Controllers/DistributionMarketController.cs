...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class DistributionMarketController : BaseMobileMemberController
    {
        ...
        public JsonResult ProductList(string skey, int page, long categoryId = 0, long shopId = 0, int sort = 0)
        {
            ...
            QueryPageModel<ProductBrokerageInfo> datasql = _iDistributionService.GetDistributionProducts(query);
        ...
        public ActionResult ProductDetail(string id = "")
        {
            ...
            var shopService = ServiceApplication.Create<IVShopService>();
            string price = "";
            ...
            shop = ServiceApplication.Create<IShopService>().GetShop(product.ShopId);
            ...
            detailModel.ProductNum = ServiceApplication.Create<IProductService>().GetShopOnsaleProducts(product.ShopId);
            bool isFavorite;
            if (CurrentUser == null)
                isFavorite = false;
            else
                isFavorite = ServiceApplication.Create<IProductService>().IsFavorite(product.Id, CurrentUser.Id);
            ...
            var bonus = ServiceApplication.Create<IShopBonusService>().GetByShopId(shop.Id);
            if (bonus != null)
            {
            ...
            detailModel.CashDepositsObligation = ServiceApplication.Create<ICashDepositsService>().GetCashDepositsObligation(product.Id);
            var cashDepositModel = ServiceApplication.Create<ICashDepositsService>().GetCashDepositsObligation(product.Id);
        ...
        public ActionResult ShopDetail(long id)
        {
            ...
            model.ShopLogo = shareInfo.DisShopBanner;
            if (CurrentUser != null)
            {
        ...
        public JsonResult ShopList(string skey, int page, long sort = 0)
        {
            ...
            QueryPageModel<DistributionShopModel> datasql = _iDistributionService.GetShopDistributionList(query);

            List<DistributionShopModel> datalist = datasql.Models.ToList();
            return SuccessResult<dynamic>(data: datalist);
        }
        #endregion
