...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class BranchProductController : BaseMobileTemplatesController
    {
        ...
        public BranchProductController(IShopService iShopService, IVShopService iVShopService, IProductService iProductService,
            ...
            , IDistributionService iDistributionService, ITypeService iTypeService, IMessageService iMessageService, IShopBranchService iShopBranchService
            )
        {
        ...
        public JsonResult GetNeedRefreshProductInfo(long id = 0, long shopId = 0, long bid = 0)
        {
            ...
            {
                salecount = ShopBranchApplication.GetProductSaleCount(bid, productId, dtNow.AddDays(-30).Date, dtNow),
                ...
                vShopId = vShopId,
                stock = branchskuList.Sum(a => a.Stock),
                cartCount = cartcount,
        ...
        /// <returns></returns>
        public ActionResult Detail(string id = "", long partnerid = 0, int nojumpfg = 0, string shopBranchId = "")
        {
            var productservice = _iProductService;
            long gid = 0;
            #region 商品Id不合法
            ...
            var shopBranchInfo = _iShopBranchService.GetShopBranchById(branchId);
            if (shopBranchInfo == null || shopBranchInfo.Status == ShopBranchStatus.Freeze)
            {
            ...
            var branchskuList = _iShopBranchService.GetSkus(shopBranchInfo.ShopId, new List<long> { branchId });
            if (branchskuList == null || branchskuList.Count <= 0)
            {
                throw new HimallException("很抱歉，您查看的门店没有该商品，可能已下架。");
            }
            ...
            ////限时购商品跳转
            //var ltmbuy = ServiceApplication.Create<ILimitTimeBuyService>().GetLimitTimeMarketItemByProductId(gid);
            //if (ltmbuy != null)
            //{
            ...
            //}
            #endregion
            ProductManagerApplication.GetWAPBranchHtml(gid, branchId);
            string urlHtml = "/Storage/Products/Statics/" + id + "-" + branchId + "-wap-branch.html";
            //统计商品浏览量、店铺浏览人数
        ...
        public ActionResult Details(long id = 0, long partnerid = 0, int nojumpfg = 0, string shopBranchId = "")
        {
            ...
            var comm = ServiceApplication.Create<ICommentService>().GetCommentsByProductId(gid);
            ...
            //var comments = ServiceApplication.Create<ICommentService>().GetComments(new CommentQuery
            //{
        ...
        public ActionResult GetStock(string skuId, long bid)
        {
            ...
            if (shopBranchs == null)
                throw new HimallException("很抱歉，您查看的门店不存在。");
            //判断门店库存和状态
            var branchSkuInfo = ShopBranchApplication.GetSkusByIds(bid, new string[] { skuId });
            ...
            var status = 0;
            if (product.AuditStatus == ProductInfo.ProductAuditStatus.Audited && product.SaleStatus == ProductInfo.ProductSaleStatus.OnSale && branchSkuInfo.FirstOrDefault().Status == ShopBranchSkuStatus.Normal)
            {
                status = 1;
            }
            return SuccessResult<dynamic>(data: new
        ...
        public ActionResult ShowSkuInfo(long id, long bid)
        {
            ...
            if (shopBranchInfo == null)
            {
                throw new Himall.Core.HimallException("门店不存在");
            }
            var branchskuList = _iShopBranchService.GetSkus(shopBranchInfo.ShopId, new List<long> { bid });
            if (branchskuList == null || branchskuList.Count <= 0)
            {
            ...
            if (!branchskustates)
            {
                throw new Himall.Core.HimallException("门店商品已下架");
            }
            ...
            model.SizeAlias = sizeAlias;
            model.VersionAlias = versionAlias;
            if (product.SKUInfo != null && product.SKUInfo.Count() > 0 && branchskuList.Count > 0)
            {
                long colorId = 0, sizeId = 0, versionId = 0;
        ...
        public JsonResult ApplyDistribution(long id)
        {
            ...
            if (setting == null)
            {
                return ErrorResult("平台未设置招募审核！");
            }
            if (setting.MustRealName || setting.MustAddress)
            {
                return ErrorResult(code: -1);
            }
            string mobile = "";
            ...
            return Json<dynamic>(result.success, result.msg, code: result.status);
        }
        #endregion
        ...
        public JsonResult GetStroreInfo(long shopId, long productId, string fromLatLng = "")
        {
            ...
            var shopBranch = shopbranchs.OrderBy(p => p.Distance).Take(1).FirstOrDefault<Himall.DTO.ShopBranch>();  //商家下有该产品的且距离用户最近的门店
            var result = new
            {
