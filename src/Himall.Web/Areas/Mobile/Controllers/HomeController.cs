...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class HomeController : BaseMobileTemplatesController
    {
        ...
        {
            var homeProducts = ServiceApplication.Create<IMobileHomeProductsService>().GetMobileHomePageProducts(0, Core.PlatformType.WeiXin).OrderBy(item => item.Sequence).ThenByDescending(o => o.Id).Skip((page - 1) * pageSize).Take(pageSize);
