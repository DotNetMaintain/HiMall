    ...
    public class ShopBranchController : BaseMobileTemplatesController
    {
        ...
        public JsonResult List(int pageSize, int pageNo, string fromLatLng, string shopId)
        {
            ...
            if (query.FromLatLng.Split(',').Length != 2)
                return Json(new Result { success = false, msg = "无法获取您的当前位置，请确认是否开启定位服务！" }, JsonRequestBehavior.AllowGet);
            ...
            if (!string.IsNullOrWhiteSpace(shopId))//如果传入了商家ID，则只取商家下门店
            {
                ...
                if (query.ShopId <= 0)
                    return Json(new Result { success = false, msg = "无法定位到商家！" }, JsonRequestBehavior.AllowGet);
            }
            else//否则取用户同城门店
            {
                ...
                if (string.IsNullOrWhiteSpace(city))
                    return Json(new Result { success = false, msg = "无法定位到城市！" }, JsonRequestBehavior.AllowGet);
        ...
        public JsonResult GetShopBranchProductAndSaleCount(string ids)
        {
            ...
            var result = idArray.Select(e =>
            {
                ShopBranchProductQuery query = new ShopBranchProductQuery();
                query.PageSize = int.MaxValue;
                query.PageNo = 1;
                query.OrderKey = 3;
                query.shopBranchId = long.Parse(e);
                query.ShopBranchProductStatus = ShopBranchSkuStatus.Normal;
                var pageModel = ShopBranchApplication.GetShopBranchProducts(query);
                var dtNow = DateTime.Now;
                var saleCount = ShopBranchApplication.GetShopBranchSaleCount(query.shopBranchId.Value, dtNow.AddDays(-30).Date, dtNow);
                var cartQuantity = memberCartInfo.Items.Where(c => c.ShopBranchId.HasValue && c.ShopBranchId.Value == query.shopBranchId && c.Status == 0).Sum(c => c.Quantity);

                return new
                {
                    saleCount = pageModel.Models.Sum(p => p.SaleCounts),
                    saleCountByMonth = saleCount,
                    cartQuantity = cartQuantity,
                    products = pageModel.Models.OrderByDescending(p => p.SaleCounts).ThenByDescending(p => p.Id).Take(4).Select(p => new Model.ProductInfo
                    {
                        Id = p.Id,
                    ...
                    branchId = e,
                    CommentScore = ShopBranchApplication.GetServiceMark(long.Parse(e)).ComprehensiveMark//综合评分
                };
            });
        ...
        public JsonResult ProductList(int pageSize, int pageNo, string productId, string shopCategoryId, string shopId, string shopBranchId, string keyWords, int type)
        {
            ...
            var dtNow = DateTime.Now;
            var pageModel = ShopBranchApplication.GetShopBranchProductsMonth(query, dtNow.AddDays(-30).Date, dtNow);//普通分页           
            ...
            {
                var comment = CommentApplication.GetProductCommentCountAggregate(productId: item.Id,
                    shopBranchId: shopBrandId);
                return new
                {
                    Id = item.Id,
                    ProductName = item.ProductName,
                    ProductTypeId = item.Himall_Categories.Id,
                    ProductType = item.Himall_Categories.Name,
            ...
            return SuccessResult<dynamic>(data: new { Models = product, TopModels = topproduct, Total = pageModel.Total });
            //return Json(new { Success = true, Models = product, TopModels = topproduct, Total = pageModel.Total }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetLoadCoupon(long shopid)
        {
            ...
            if (result.Count() > 0 && couponSetList.Count() > 0)
            {
                var couponList = result.ToArray().Where(item => couponSetList.Contains(item.Id)).ToList();
        ...
        public JsonResult GetUserShippingAddressesList()
        {
            if (CurrentUser == null)
            {
                return Json(new Result { success = false, msg = "nologin" }, JsonRequestBehavior.AllowGet);
            }
            ...
            return Json(new Result() { success = true, data = json }, JsonRequestBehavior.AllowGet);
        }
        #region Search
        ...
        public JsonResult SearchShopBranchProductAndSaleCount(string ids, string keyWords)
        {
            ...
            var idArray = ids.Split(',');
            //购物车商品数
            var cartItemCount = ShopBranchApplication.GetShopBranchCartItemCount(userId, idArray.Select(e=> long.Parse(e)));
            var result = idArray.Select(e =>
            {
                ShopBranchProductQuery query = new ShopBranchProductQuery();
                query.PageSize = int.MaxValue;
                ...
                //过滤购物车 无效商品
                var cartQuantity = cartItemCount.ContainsKey(query.shopBranchId.Value) ? cartItemCount[query.shopBranchId.Value] : 0;
                ...
                return new
                {
                    ...
                    saleCountByMonth = saleCount,
                    cartQuantity = cartQuantity,
                    products = pageModel.Models.OrderByDescending(p => p.SaleCounts).ThenByDescending(p => p.Id).Select(p => new Model.ProductInfo
                    {
                        Id = p.Id,
                        ImagePath = p.ImagePath,
                        MinSalePrice = p.MinSalePrice,
        ...
        public ActionResult Tags(int? id)
        {
            ...
            if (!id.HasValue)
            {
                throw new Core.HimallException("非法参数！");
            }
            var tag = ShopBranchApplication.GetShopBranchTagInfo(id.Value);
            if (null == tag)
            {
                throw new Core.HimallException("门店标签已经被管理员删除 ！");
            }
            return View(tag);
        }
        ...
        public JsonResult TagsSearch(int pageSize, int pageNo, string fromLatLng, string shopId, long tagsId)
        {
            ...
            return Json(new { success = true, data = new { Models = homepageBranchs, Total = shopBranchs.Total, CityInfo = new { Id = cityInfo.Id, Name = cityInfo.Name }, CurrentAddress = currentPosition } }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetStoreByProductAndSaleCount(string ids, long? productId)
        {
            ...
            var result = idArray.Select(e =>
            {
                ...
                {
                    var product = pageModel.Models.FirstOrDefault(n => n.Id == productId.Value);
                ...
                var dtNow = DateTime.Now;
                var saleCount = ShopBranchApplication.GetShopBranchSaleCount(query.shopBranchId.Value, dtNow.AddDays(-30).Date, dtNow.Date);
                var cartQuantity = memberCartInfo.Items.Where(c => c.ShopBranchId.HasValue && c.ShopBranchId.Value == query.shopBranchId).Sum(c => c.Quantity);
                ...
                return new
                {
                    ...
                    saleCountByMonth = saleCount,
                    cartQuantity = cartQuantity,
                    products = pageModel.Models.Select(p => new Model.ProductInfo
                    {
                        Id = p.Id,
                        ImagePath = p.ImagePath,
                        MinSalePrice = p.MinSalePrice,
