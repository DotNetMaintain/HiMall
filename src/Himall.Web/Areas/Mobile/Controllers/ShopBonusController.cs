    ...
    public class ShopBonusController : BaseMobileTemplatesController
    {
        ...
        public ActionResult Index(long id)
        {
            ...
            if (obj.State == ShopReceiveStatus.CanReceive)
            {
                ...
                return Redirect("/m-weixin/ShopBonus/Completed/" + model.Id + "?openId=" + wxUserInfo.openid + "&price=" + obj.Price + "&user=" + obj.UserName + "&grantid=" + id + "&rid=" + obj.Id + "&wxhead=" + wxUserInfo.headimgurl + "&host=" + host);
            }
            else if (obj.State == ShopReceiveStatus.CanReceiveNotUser)
            {
                return Redirect("/m-weixin/ShopBonus/CompletedNotUser/" + model.Id + "?openId=" + wxUserInfo.openid + "&price=" + obj.Price + "&grantid=" + id + "&rid=" + obj.Id + "&wxhead=" + wxUserInfo.headimgurl + "&host=" + host);
            }
            else if (obj.State == ShopReceiveStatus.Receive)
            {
                return Redirect("/m-weixin/ShopBonus/HasReceive/" + model.Id + "?openId=" + wxUserInfo.openid + "&grantid=" + id + "&wxhead=" + wxUserInfo.headimgurl + "&host=" + host);
            }
        ...
        public ActionResult Completed(long id, string openId = "", decimal price = 0, string user = "", long grantid = 0, long rid = 0, string wxhead = "", string host = "")
        {
            ...
            model.ShareImg = HimallIO.GetRomoteImagePath(model.ShareImg);
