...
{
    public class VShopController : BaseMobileMemberController
    {
        ...
        public ActionResult Detail(long id, int? couponid, int? shop, bool sv = false, int ispv = 0, string tn = "")
        {
            ...
            if (null != s && s.ShopStatus == ShopInfo.ShopAuditStatus.HasExpired)
                throw new HimallException("此店铺已过期");
            if (null != s && s.ShopStatus == ShopInfo.ShopAuditStatus.Freeze)
                throw new HimallException("此店铺已冻结");
            if(vshop.State== VShopInfo.VshopStates.Close)
            {
        ...
        public ActionResult Introduce(long id)
        {
            ...
            ViewBag.IsFavorite = isFavorite;
            var mark = Framework.ShopServiceMark.GetShopComprehensiveMark(vshop.ShopId);
        ...
        {
            var productList = ServiceApplication.Create<IMobileHomeProductsService>().GetMobileHomePageProducts(shopId, Himall.Core.PlatformType.WeiXin).OrderBy(item => item.Sequence).ThenByDescending(o => o.Id).Skip((pageNo - 1) * pageSize).Take(pageSize);
        ...
        )
        {
            ...
            var productsResult = ServiceApplication.Create<IProductService>().SearchProduct(model);
        ...
        public ActionResult Category(long vShopId)
        {
            ...
            var bizCategories = ServiceApplication.Create<IShopCategoryService>().GetShopCategory(vshopInfo.ShopId).ToList();
            var shopCategories = GetSubCategories(bizCategories, 0, 0);
            ViewBag.VShopId = vShopId;
        ...
        public ActionResult VShopHeader(long id)
        {
            ...
            //轮播图
            var slideImgs = ServiceApplication.Create<ISlideAdsService>().GetSlidAds(vshop.ShopId, Model.SlideAdInfo.SlideAdType.VShopHome).ToList();


            var homeProducts = ServiceApplication.Create<IMobileHomeProductsService>().GetMobileHomePageProducts(vshop.ShopId, Himall.Core.PlatformType.WeiXin).OrderBy(item => item.Sequence).ThenByDescending(o => o.Id).Take(8);
