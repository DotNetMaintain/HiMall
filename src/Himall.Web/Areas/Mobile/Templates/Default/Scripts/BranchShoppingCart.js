    ...
    $(sbottom).find("button").click(function () {
        ...
        checkLogin(returnHref, function () {
            if (!$(this).hasClass("disabled")) {
                ...
                var url = '/common/site/pay?area=mobile&platform=Wap&controller=BranchOrder&action=SubmiteByCart&cartItemIds={0}'.format(itemId.substr(0, itemId.length - 1));
...
function LoadBottomHtml(data) {
    ...
    $.each(products, function () {
        ...
        if (this.status == 0) {
            ...
            //                "    <span class=\"disable-reason\">" + (this.status == "1" ? "已失效" : "库存不足") + "</span>" +
            //                "</div></li>";
