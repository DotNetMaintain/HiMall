...
function justBuy(callBack) {
    ...
    if (len === $(".spec").length) {
        ...
        $.post('../CheckLimitTimeBuy', { skuIds: sku, counts: num }, function(result) {
            if (result.success) {
                var url = '/common/site/pay?area=mobile&platform=Wap&controller=order&action=submit&skuIds={0}&counts={1}&isLimit=1&returnUrl={2}'.format(sku, num, returnUrl);
                location.href = url;
                ...
                $.dialog.errorTips("亲，限购" + result.data.maxSaleCount + "件，您最多能买" + result.data.remain + "件");
            }
        });
    } else {
