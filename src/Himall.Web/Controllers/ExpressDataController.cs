...
namespace Himall.Web.Controllers
{
    public class ExpressDataController : BaseController
    {
        ...
        public JsonResult Search(string expressCompanyName, string shipOrderNumber)
        {
            ...
            var expressName = "";
            var expressUrl = "";
            var settting = SiteSettingApplication.GetSiteSettings();
        ...
        public JsonResult SaveExpressData(string param)
        {
            ...
            try
            {
                ...
                return Json(new { result = true, returnCode = 200, message = "成功" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Core.Log.Error("保存快递信息错误：" + ex.Message + param);
                return Json(new { result = false, returnCode = 500, message = "服务器错误" + ex.Message }, JsonRequestBehavior.AllowGet);
            }
        }
    }
}