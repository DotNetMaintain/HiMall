 ...
 * QQ:673921852
 **/
//var URL='http://himallupdate.himall.kuaidiantong.cn/',
var URL='https://himallhttps.himall.kuaidiantong.cn/',
	AppName='HiMall商家',
	app_key='himalltest';
...
//定义自动打印客户端方法
function autoPrint(orderId,printCount,bleObj){
	//alert("app.js中自动打印方法执行");
	for(var i=1;i<=printCount;i++){
		PrintOrder(orderId,bleObj);
	}
}
...
function PrintForamt(bleObj, data) {
			var printText = printInit(); //初始化
				...
				if(isJoin){
					printText += res.ShopName + "\n\n";
				}
				else{
					printText += res.ShopBranchName + "\n\n";
				}
