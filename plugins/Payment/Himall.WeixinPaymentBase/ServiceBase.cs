...
namespace Himall.WeixinPaymentBase
{
    public class ServiceBase : PaymentBase<Config>
    {
                  ...
                  //AppId
                   ...
                   new  FormData.FormItem(){
                    ...
                    new  FormData.FormItem(){
                     ...
                     IsRequired = false,
                      Type= FormData.FormItemType.text,
                      Value=config.Main_Mch_ID
                   },
                     new  FormData.FormItem(){
                     DisplayName = "服务商AppID",
                     Name = "Main_AppId",
                     IsRequired = false,
                      Type= FormData.FormItemType.text,
                      Value=config.Main_AppId
                   }
                }
            };
            return formData;
        ...
        public void SetFormValues(IEnumerable<KeyValuePair<string, string>> values)
        {
            ...
            //如果包含了参数Main_AppId和Main_Mch_ID则为服务商模式
            var Main_AppId = values.FirstOrDefault(item => item.Key == "Main_AppId");
            var Main_Mch_ID = values.FirstOrDefault(item => item.Key == "Main_Mch_ID");
            if (Main_Mch_ID.Key == "Main_Mch_ID")
            {
                if (string.IsNullOrWhiteSpace(Main_Mch_ID.Value))
                    throw new PluginConfigException("服务商商户ID不能为空");
            }
            if (Main_AppId.Key== "Main_AppId")
            {
                if (string.IsNullOrWhiteSpace(Main_AppId.Value))
                    throw new PluginConfigException("服务商AppID不能为空");
            }
            ...
            oldConfig.Main_AppId = Main_AppId.Value;
            oldConfig.Main_Mch_ID = Main_Mch_ID.Value;
