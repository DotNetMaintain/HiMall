...
namespace Himall.Plugin.Payment.WeiXinPay_H5
{
    public class Service : ServiceBase, IPaymentPlugin
    {
        public string GetRequestUrl(string returnUrl, string notifyUrl, string orderId, decimal totalFee, string productInfo, string openId = null)
        {
            ...
            //用户的公网ip，不是商户服务器IP
            //string exter_invoke_ip = Himall.Core.Helper.WebHelper.GetIP();
            ...
            if (returnCode == "FAIL")//失败
                throw new ApplicationException("微信预支付失败：" + res.Element("xml").Element("return_msg").Value);

            string resultCode = res.Element("xml").Element("result_code").Value;
            if (resultCode == "FAIL")
                throw new ApplicationException("微信预支付失败：" + res.Element("xml").Element("err_code_des").Value);

            string mweb_url = res.Element("xml").Element("mweb_url").Value;
            if(string.IsNullOrEmpty(mweb_url))
            {
                throw new ApplicationException("微信支付失败：支付跳转url不能为空！");
            }
